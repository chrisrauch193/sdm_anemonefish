...
# Assumes 06a produced host predictions for ALL scenarios using PCA.
# V3: Aligned output with paper structure.
#-------------------------------------------------------------------------------
cat("--- Running Script 06d: Run Anemonefish Combined (Env PCA + Biotic) SDMs (v3 Paper Structure) ---\n")
...
log4r::info(logger, "--- Starting Script 06d: Run Anemonefish Combined (Env PCA + Biotic) SDMs (v3 Paper Structure) ---")
...
if(!config$use_pca_predictors) {
  log4r::fatal(logger, "Script 06d (Combined) requires config$use_pca_predictors=TRUE.")
...
predictor_type_suffix <- "_combined_pca" # Suffix for intermediate files and logs
paper_file_suffix <- "_EFN" # Suffix for paper structure CV/VI files, mimicking "_EFN" for combined
...
# Look for host predictions in YOUR intermediate dir
host_predictions_dir_intermediate <- file.path(config$predictions_dir, paste0(host_group_name, host_predictor_type_suffix))
if(!dir.exists(host_predictions_dir_intermediate)) { log4r::fatal(logger, paste("Intermediate Host predictions directory not found:", host_predictions_dir_intermediate)); stop("Host predictions missing.") }
...
for (scenario in scenarios_to_process) {
  ...
  for (i in 1:nrow(host_species_df)) {
    ...
    pred_file_path <- file.path(host_predictions_dir_intermediate, paste0("sdm_prediction_", host_sci_name_sanitized, "_", scenario, host_predictor_type_suffix, ".tif"))
  ...
  if(length(all_host_prediction_paths[[scenario]]) == 0 && missing_files_scenario > 0){
    ...
    log4r::warn(logger, paste("Found", length(all_host_prediction_paths[[scenario]]), "host files, but", missing_files_scenario, "missing for scenario:", scenario))
  } else {
    log4r::info(logger, paste("Found all expected host prediction files for scenario:", scenario))
  }
}
if (missing_host_preds_any_scenario) { log4r::warn(logger, "One or more required host predictions missing for some scenarios.") }

# --- 7. Output Directories (Already created via config.R) ---
# Paper Structure Dirs (from config)
paper_cv_dir_biotic <- config$paper_results_biotic_dir # For combined CV
paper_vi_dir_biotic <- config$paper_vi_biotic_dir      # For combined VI
...
log4r::debug(logger, paste("Paper CV output directory:", paper_cv_dir_biotic))
log4r::debug(logger, paste("Paper VI output directory:", paper_vi_dir_biotic))
log4r::debug(logger, paste("Paper Prediction output directory:", paper_pred_base_dir))
...
process_species_sdm_combined <- function(species_row, config, env_predictor_paths_list, all_host_prediction_paths, associations_df,
  ...
  slog("INFO", "--- Starting Combined (Env PCA + Biotic) processing ---")
  ...
  if (is.null(tuning_env_path) || !file.exists(tuning_env_path)) { msg <- paste0(log_prefix, "Skipping: Env (PCA) stack path missing for tuning."); slog("ERROR", msg); return(list(status = "error_tuning_env_predictors", species = species_name, occurrence_count = NA, message = msg)) }
  ...
  if (length(host_paths_for_tuning) == 0) { msg <- paste0(log_prefix, "Skipping: No host paths found for tuning."); slog("ERROR", msg); return(list(status = "error_tuning_biotic_predictors", species = species_name, occurrence_count = NA, message = msg)) }
  
  host_rasters_tuning_list <- list()
  for(host_name in names(host_paths_for_tuning)){
    host_path <- host_paths_for_tuning[[host_name]]; host_rast <- tryCatch(terra::rast(host_path), error=function(e){slog("WARN", paste("Failed load host", basename(host_path),":",e$message)); NULL})
    if(!is.null(host_rast)){ if (!terra::compareGeom(reference_geom_tuning, host_rast, stopOnError=FALSE)) { slog("DEBUG", paste("Resampling host", basename(host_path))); host_rast <- tryCatch(terra::resample(host_rast, reference_geom_tuning, method="bilinear"), error=function(e){slog("ERROR", "Failed resample host"); NULL}) }
      if(!is.null(host_rast)){ names(host_rast) <- host_name; host_rasters_tuning_list[[host_name]] <- host_rast }}
  }
  if (length(host_rasters_tuning_list) == 0) { msg <- paste0(log_prefix, "Skipping: Failed load/resample hosts for tuning."); slog("ERROR", msg); return(list(status = "error_tuning_biotic_load", species = species_name, occurrence_count = NA, message = msg)) }
  ...
  tuning_combined_stack <- tryCatch(c(tuning_env_stack, max_host_suitability_tuning), error = function(e) { slog("ERROR", "Failed combine stacks."); NULL })
  if(is.null(tuning_combined_stack)) { msg <- paste0(log_prefix, "Skipping: Failed combine stacks."); slog("ERROR", msg); return(list(status = "error_tuning_combine_stack", species = species_name, occurrence_count = NA, message = msg)) }
  slog("INFO", paste("Tuning stack created with:", paste(names(tuning_combined_stack), collapse=", ")))
  ...
  # CV results path (goes into the paper 'antcontrasts' dir)
  paper_cv_results_file <- file.path(paper_cv_dir, paste0("CV_Results_", species_name_sanitized, paper_file_suffix, ".csv"))
  # VI results path (goes into the paper 'vi_ants' dir)
  paper_vi_results_file <- file.path(paper_vi_dir, paste0("vi_", species_name_sanitized, paper_file_suffix, ".csv"))
  ...
  # Intermediate tuning results path
  ...
  if (!config$force_rerun$run_biotic_sdms && file.exists(final_model_file_intermediate)) {
    slog("INFO", "Skipping tuning/training: Final intermediate combined model exists.")
  ...
  if (!load_existing_model) {
    ...
    tuning_output_list <- run_sdm_tuning_kfold(occs_coords, tuning_combined_stack, background_points, config, logger = NULL, species_name)
  ...
  if (config$run_variable_importance && !is.null(final_model)) {
    ...
    if (!is.null(tuning_results_object) && inherits(tuning_results_object@models[[1]], "SDMmodelCV")) {
      ...
      if (!is.null(tuning_combined_stack) && !is.null(occs_coords)) { # Use combined stack here
        background_points_vi <- generate_sdm_background(tuning_combined_stack, config$background_points_n, config, logger = NULL, seed = species_aphia_id + 1)
        if (!is.null(background_points_vi)){ test_swd_data <- tryCatch(SDMtune::prepareSWD(species = species_name, p = occs_coords, a = background_points_vi, env = tuning_combined_stack), error=function(e) NULL); rm(background_points_vi); gc() }
      }
    }
  ...
  rm(tuning_combined_stack, tuning_results_object); gc()
  
  # --- Prediction Loop ---
  predictions_made = 0; prediction_errors = 0
  scenarios_to_predict <- names(env_predictor_paths_list) # Predict for all scenarios where we have PCA env data
  ...
  if (!is.null(final_model)) {
    for (pred_scenario in scenarios_to_predict) {
      ...
      if (pred_scenario == "current") {
        ...
        target_pred_filename <- paste0("mean_pred_", species_name_sanitized, paper_file_suffix, ".tif") # Suffix like _EFN
      } else {
        ...
        target_pred_filename <- paste0("mean_pred_", species_name_sanitized, "_", ssp_code, "_", time_tag_clean, paper_file_suffix, ".tif")
      }
      ...
      pred_env_stack <- tryCatch(terra::rast(pred_env_path), error = function(e) { slog("WARN", "Error load Env PCA stack"); NULL })
      ...
      if (length(host_paths_for_pred) == 0) { slog("WARN", "No host paths"); prediction_errors <- prediction_errors + 1; next }
      ...
      for(host_name in names(host_paths_for_pred)){
        ...
        if(!is.null(host_rast)){ if (!terra::compareGeom(reference_geom_pred, host_rast, stopOnError=FALSE)) { slog("DEBUG", paste("Resampling host", basename(host_path))); host_rast <- tryCatch(terra::resample(host_rast, reference_geom_pred, method="bilinear"), error=function(e){slog("ERROR", "Failed resample host");NULL}) }
      ...
      # 3. Combine Env + Biotic for prediction
      prediction_combined_stack <- tryCatch(c(pred_env_stack, max_host_suitability_scenario), error=function(e){slog("WARN", "Error combining stacks"); NULL})
      if(is.null(prediction_combined_stack)) { prediction_errors <- prediction_errors + 1; next }
      # Ensure predictor names match EXACTLY what the model was trained on
      expected_names <- names(final_model@data@data) |> setdiff(c("species", "pa"))
      names(prediction_combined_stack) <- expected_names
      ...
      rm(pred_env_stack, host_stack_scenario, max_host_suitability_scenario, prediction_combined_stack, host_rasters_pred_list); gc()
    } # End prediction scenario loop
  } else { prediction_errors <- length(scenarios_to_predict); msg <- paste0(log_prefix, "Skipping preds: Final model unavailable."); slog("ERROR", msg); status <- if(load_existing_model) "error_loading_model" else "error_training"; return(list(status = status, species = species_name, occurrence_count = occurrence_count_after_thinning, message = msg)) }
  
  # --- Prepare return status ---
  final_status <- "success"; status_message <- paste0(log_prefix, "Finished Combined. Occs:", occurrence_count_after_thinning, ". Preds attempted:", length(scenarios_to_predict), ". Made:", predictions_made, ". Errors/Skipped:", prediction_errors, ".")
...
results_list <- progressr::with_progress({ furrr::future_map(1:nrow(species_df), ~{ process_species_sdm_combined( species_row = species_df[.x, ], config = config, env_predictor_paths_list = env_predictor_paths_list, all_host_prediction_paths = all_host_prediction_paths, associations_df = associations_df, paper_cv_dir = config$paper_results_biotic_dir, paper_vi_dir = config$paper_vi_biotic_dir, paper_pred_base_dir = config$paper_pred_base_dir, paper_pred_future_base_dir = config$paper_pred_future_base_dir, intermediate_results_dir = intermediate_results_dir, intermediate_models_dir = intermediate_models_dir, species_log_dir = config$species_log_dir, predictor_type_suffix = predictor_type_suffix, paper_file_suffix = paper_file_suffix, occurrence_dir = occurrence_dir ) }, .options = furrr_options(seed = TRUE, scheduling = 1.0)) })
...
log4r::info(logger, "--- Processing Results Summary (Anemonefish Combined Models) ---")
...
log4r::info(logger, paste("--- Overall Summary (Anemonefish Combined Models) ---"))
...
if (length(occurrence_counts) > 0) {
  ...
  cat("\n--- Final Occurrence Counts (Anemonefish Combined, after cleaning/thinning) ---\n"); print(occ_count_df_print)
} else { log4r::warn(logger, "No occurrence counts recorded.") }

future::plan(future::sequential); gc(full=TRUE)
log4r::info(logger, "--- Script 06d (v3 Paper Structure) finished. ---")
#-------------------------------------------------------------------------------