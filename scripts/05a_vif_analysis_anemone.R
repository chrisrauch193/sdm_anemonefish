...
#-------------------------------------------------------------------------------
cat("--- Running Script 05a: VIF/Correlation Analysis for Anemone Hosts ---\n")
...
# --- 3. *** USER DEFINED: INITIAL VARIABLE LIST (Anemone) *** ---
# Define the CORE variables (baseline format) you want to start with.
...
core_vars_anemone <- c(
  ...
  #"thetao_baseline_depthmax_ltmin", # Example: Commented out
...
for (scenario in config$env_scenarios) {
  ...
  cat("  Variables remaining after NA/zero-var removal:", paste(names(env_values_df), collapse=", "), "\n")
  ...
  tryCatch({
    ...
    plot_vif_results_original(vif_results, save_path = vif_plot_path, display_lookup = config$core_var_display_names)
  ...
  tryCatch({
    plot_correlation_results_original(env_values_df, save_path = cor_plot_path, display_lookup = config$core_var_display_names)
    ...
    if(nrow(high_cor_pairs_indices) > 0){
      ...
      display_names_in_matrix <- sapply(rownames(cor_matrix_vals), config$get_display_name, lookup = config$core_var_display_names) # Use from config
...
cat("\n--- Script 05a finished. Review logs/plots and iterate on 'core_vars_anemone' list if needed. ---\n")
#-------------------------------------------------------------------------------