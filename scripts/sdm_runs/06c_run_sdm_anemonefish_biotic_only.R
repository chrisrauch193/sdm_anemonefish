...
#-------------------------------------------------------------------------------
# Run SDMs for Anemonefish Species using ONLY Max Host Suitability
# (SDMtune Workflow). Includes a DUMMY constant env layer to satisfy SDMtune.
# Includes Parallel Processing, Logging, Progress, Species Logs, SAC thinning, OBIS background.
# Saves outputs to target structure. Assumes 06a produced host predictions.
# v8 - Biotic Only with Dummy Env Layer.
#-------------------------------------------------------------------------------
cat("--- Running Script 06c: Run Anemonefish Biotic-Only SDMs (v8 - Dummy Env Layer) ---\n")
...
log4r::info(logger, "--- Starting Script 06c: Run Anemonefish Biotic-Only SDMs (v8) ---")
...
host_group_name <- "anemone" # Group providing the biotic predictor

# Define the specific suffix for this run type
predictor_type_suffix <- "_biotic_only"
# Suffix of the host predictions generated by 06a to load
host_predictor_type_suffix <- "_pca" # Host predictions were based on PCA models

log4r::info(logger, paste("--- Processing Group:", group_name, "---"))
log4r::info(logger, paste("--- Using Predictors: Biotic Only (Max Host Suitability", host_predictor_type_suffix, ") + Dummy Env Layer ---"))


# --- 6. Load Environmental Predictor Paths (PCA) - NEEDED FOR REFERENCE GEOMETRY ONLY ---
# We still need one file (e.g., current PC1) to get the base geometry for the dummy layer
pca_paths_rds <- config$pca_raster_paths_rds_path
if (is.null(pca_paths_rds) || !is.character(pca_paths_rds) || nchar(pca_paths_rds) == 0 || !file.exists(pca_paths_rds)) { log4r::fatal(logger, "PCA paths RDS missing/invalid (needed for reference geometry)."); stop("PCA paths RDS invalid.") }
...
scenarios_to_process <- names(env_predictor_paths_list) # Get list of all available scenarios
# Get path for the reference geometry (use current PC1)
reference_geom_path <- env_predictor_paths_list[["current"]]
if(is.null(reference_geom_path) || !file.exists(reference_geom_path)) { log4r::fatal(logger, "Path to 'current' PCA stack missing (needed for reference geometry)."); stop("Reference geometry path missing.")}
log4r::info(logger, paste("Reference geometry for dummy layer will be based on:", basename(reference_geom_path)))
...
for (scenario in scenarios_to_process) {
  ...
  if(length(all_host_prediction_paths[[scenario]]) == 0 && missing_files_scenario > 0){ log4r::error(logger, paste("FATAL: No host predictions found for ANY species in target location for scenario:", scenario)); missing_host_preds_overall <- TRUE }
...
# Use the NEW suffix "_biotic_only"
...
intermediate_models_dir <- file.path(base_intermediate_model_path, paste0(group_name, predictor_type_suffix)) # e.g., .../anemonefish_biotic_only
intermediate_results_dir <- file.path(base_intermediate_results_path, paste0(group_name, predictor_type_suffix)) # e.g., .../anemonefish_biotic_only
tryCatch({
...
# --- 11. Define Function to Process Single Species (Biotic Only v8) ---
process_species_sdm_biotic_only <- function(species_row, config, reference_geom_path, all_host_prediction_paths, associations_df,
  ...
  slog("INFO", "--- Starting Biotic-Only processing (v8 - Dummy Env) ---")
  ...
  # --- Prepare GLOBAL predictor stack for tuning scenario (Biotic + Dummy) ---
  slog("DEBUG", "Preparing GLOBAL Biotic+Dummy predictor stack for tuning scenario:", tuning_scenario)
  tuning_predictor_stack_global <- NULL
  tryCatch({
    # Load reference geometry (e.g., from current PC1) to define extent/res/crs
    reference_raster <- terra::rast(reference_geom_path)[[1]] # Load first layer
    if(is.null(reference_raster)) stop("Failed to load reference raster for geometry.")
    
    # Get host paths for the tuning scenario
    ...
    # Load, resample (if needed), and stack host rasters
    ...
    for(host_name in names(host_paths_for_tuning)){
      ...
      if(!is.null(host_rast)){
        # Use reference_raster for comparison and resampling target
        if (!terra::compareGeom(reference_raster, host_rast, stopOnError=FALSE, res=TRUE)) {
          slog("DEBUG", paste("Resampling host", basename(host_path), "for tuning."))
          host_rast <- tryCatch(terra::resample(host_rast, reference_raster, method="bilinear"), error = function(e) { slog("ERROR", paste("Failed resample host", basename(host_path),":", e$message)); NULL })
        }
    ...
    # Create the dummy layer matching the host suitability layer's geometry
    dummy_env_layer <- max_host_suitability_tuning # Use host layer as template
    dummy_env_layer[] <- 1 # Set all values to 1
    names(dummy_env_layer) <- "dummy_env"
    slog("DEBUG", "Dummy environment layer (value=1) created.")
    
    # Combine Biotic + Dummy
    tuning_predictor_stack_global <- c(max_host_suitability_tuning, dummy_env_layer)
    slog("INFO", paste("GLOBAL Tuning predictor stack created:", paste(names(tuning_predictor_stack_global), collapse=", ")))
    rm(host_stack_tuning, max_host_suitability_tuning, host_rasters_tuning_list, reference_raster); gc() # Clean up intermediates
    
  }, error = function(e) {
    msg <- paste0("Skipping: Failed to prepare GLOBAL Biotic+Dummy tuning stack: ", e$message); slog("ERROR", msg)
    if(exists("host_stack_tuning")) rm(host_stack_tuning); if(exists("max_host_suitability_tuning")) rm(max_host_suitability_tuning); if(exists("host_rasters_tuning_list")) rm(host_rasters_tuning_list); if(exists("reference_raster")) rm(reference_raster); if(exists("dummy_env_layer")) rm(dummy_env_layer)
    gc(); return(list(status = "error_tuning_predictors", species = species_name, occurrence_count = NA, message = msg))
  })
  ...
  sac_thin_result <- thin_occurrences_by_sac(occs_coords = occs_coords_raw, predictor_stack = tuning_predictor_stack_global, config = config, logger = NULL, species_log_file = species_log_file)
  ...
  if (!load_existing_model) {
    ...
    species_specific_stack <- background_return$species_specific_stack # Use this masked Biotic+Dummy stack
  ...
  # Predict for all scenarios where host predictions exist
  scenarios_to_predict <- names(all_host_prediction_paths)
  ...
  if (!is.null(final_model)) {
    for (pred_scenario in scenarios_to_predict) {
      ...
      # --- Prepare GLOBAL Stack for Prediction Scenario (Biotic + Dummy) ---
      pred_predictor_stack_global <- NULL
      tryCatch({
        # Get host paths for this scenario
        ...
        # Load first host raster to get geometry for this scenario
        first_host_path <- host_paths_for_pred[[1]]
        reference_geom_pred <- tryCatch(terra::rast(first_host_path)[[1]], error = function(e){stop("Cannot load first host raster for reference geometry.")})
        
        # Load/Resample/Stack Hosts for this scenario
        ...
        # Create dummy layer matching this scenario's geometry
        dummy_env_layer_pred <- max_host_suitability_scenario
        dummy_env_layer_pred[] <- 1
        names(dummy_env_layer_pred) <- "dummy_env"
        
        # Combine Biotic + Dummy
        pred_predictor_stack_global <- c(max_host_suitability_scenario, dummy_env_layer_pred)
        slog("DEBUG", paste("  GLOBAL Predictor stack for prediction:", paste(names(pred_predictor_stack_global), collapse=", ")))
        rm(reference_geom_pred, host_stack_scenario, max_host_suitability_scenario, host_rasters_pred_list, dummy_env_layer_pred); gc()
        
      }, error = function(e) {
        slog("ERROR", paste("  Failed preparing GLOBAL stack for prediction:", pred_scenario, "Error:", e$message))
        if (exists("reference_geom_pred")) rm(reference_geom_pred); if (exists("host_stack_scenario")) rm(host_stack_scenario); if (exists("max_host_suitability_scenario")) rm(max_host_suitability_scenario); if (exists("host_rasters_pred_list")) rm(host_rasters_pred_list); if(exists("dummy_env_layer_pred")) rm(dummy_env_layer_pred)
  ...
  final_status <- "success"; status_message <- paste0("Finished Biotic-Only SDM. Occs:", occurrence_count_after_thinning, ". Preds attempted:", length(scenarios_to_predict), ". Made:", predictions_made, ". Errors/Skipped:", prediction_errors, ".")
...
log4r::info(logger, paste("Starting Biotic-Only SDM processing for", nrow(species_df), group_name, "species..."))

results_list <- progressr::with_progress({
  furrr::future_map(1:nrow(species_df), ~{
    process_species_sdm_biotic_only( # Call the updated function name
      species_row = species_df[.x, ],
      config = config,
      reference_geom_path = reference_geom_path, # Pass path for reference geom
      all_host_prediction_paths = all_host_prediction_paths,
      associations_df = associations_df,
      group_name = group_name,
      predictor_type_suffix = predictor_type_suffix, # Use "_biotic_only"
...
log4r::info(logger, "--- Script 06c (v8 - Biotic Only with Dummy Env) finished. ---")
#-------------------------------------------------------------------------------