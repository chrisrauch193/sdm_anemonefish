...
# --- 9. Define Function to Process Single Species (Incorporating SAC Thinning) ---
process_species_sdm <- function(species_row, config, predictor_paths_or_list, group_name, predictor_type_suffix, use_pca, occurrence_dir,
  ...
  slog("INFO", "--- Starting processing (tweaking_base_run.R with SAC thinning) ---")
  ...
  # --- Load Predictor Stack for Tuning Scenario ---
  ...
  config_for_occ_load$predictor_stack_for_thinning <- NULL # Don't pass stack for cell thinning
  config_for_occ_load$thinning_method <- NULL # Disable cell thinning
  ...
  if (is.null(occ_data_result_raw) || is.null(occ_data_result_raw$coords) || occ_data_result_raw$count < config$min_occurrences_sdm) {
    ...
    slog("WARN", msg);
    return(list(status = "skipped_occurrences", species = species_name, occurrence_count = occ_data_result_raw$count %||% 0, message = msg))
  }
  ...
  # Call the SAC thinning helper function
  sac_thin_result <- thin_occurrences_by_sac(
    occs_coords = occs_coords_raw,
    predictor_stack = tuning_predictor_stack, # Use the current scenario stack for env data
    config = config,
  ...
  # Note: occurrence_count_after_thinning now reflects the count after SAC thinning (or raw count if failed/skipped)
  ...
  if (!config$force_rerun$run_standard_sdms && file.exists(final_model_file)) {
    ...
    if(!is.null(final_model) && inherits(final_model, "SDMmodel")){
      ...
      # Need background and stack corresponding to *when the model was trained*
      # Regenerate them here based on *thinned* data
      slog("DEBUG", "Generating background points for loaded model VI/Eval...")
      background_return_eval <- generate_sdm_background_obis(occs_sf_clean, tuning_predictor_stack, config, logger=NULL, species_log_file=species_log_file, seed_offset = species_aphia_id)
      if(!is.null(background_return_eval) && !is.null(background_return_eval$background_points) && !is.null(background_return_eval$species_specific_stack)) {
        ...
        species_specific_stack_eval <- background_return_eval$species_specific_stack
        full_swd_data <- tryCatch({ SDMtune::prepareSWD(species = species_name, p = occs_coords, a = background_points_eval, env = species_specific_stack_eval, verbose = FALSE) }, error = function(e) { slog("WARN", "Failed prepareSWD for VI/Eval on loaded model:", e$message); NULL })
        rm(background_points_eval, species_specific_stack_eval); gc()
      ...
      if(file.exists(tuning_rds_file)) {
        ...
        slog("WARN", "Tuning RDS file not found, cannot log test metrics for loaded model.")
      }
      if(is.null(tuning_output)) { # Ensure tuning_output is NULL if loading failed
        tuning_output <- NULL
      }
  ...
  if (!load_existing_model) {
    ...
    # calibration_polygon <- background_return$calibration_polygon # Keep if needed later
    # quad_n_calculated <- background_return$quad_n_calculated # Keep if needed later
    ...
    if (is.null(full_swd_data)) { rm(background_points, species_specific_stack); gc(); return(list(status = "error_swd_preparation", species = species_name, occurrence_count = occurrence_count_after_thinning, message = paste0("log_prefix", "SWD prep failed."))) }
    ...
    # NOTE: run_sdm_tuning_scv uses the SWD data passed to it, which now incorporates the species_specific_stack
    tuning_output <- run_sdm_tuning_scv(occs_coords, species_specific_stack, background_points, config, logger=NULL, species_name, species_log_file=species_log_file) # Pass thinned coords
    ...
    final_model <- train_final_sdm(occs_coords, species_specific_stack, background_points, best_hypers, config, logger=NULL, species_name, species_log_file=species_log_file) # Pass thinned coords
    ...
    # Save Final Model
    if(!save_final_model(final_model, species_name_sanitized, predictor_type_suffix, config, logger=NULL, species_log_file=species_log_file)) {
      # Logged within helper
    ...
    rm(background_points); gc() # Keep species_specific_stack & full_swd_data for logging
    
  } # End if !load_existing_model
  
  
  # --- Log Metrics (Ensure this happens AFTER model training or loading) ---
  if (!is.null(final_model)) { # Only log if we have a final model
    log_final_model_metrics(
      final_model = final_model,
      full_swd_data = full_swd_data, # Pass SWD data (might be NULL if loaded model and SWD regen failed)
      tuning_predictor_stack = if(exists("species_specific_stack")) species_specific_stack else tuning_predictor_stack, # Pass stack used for training/AICc
    ...
    slog("WARN", "Final model object is NULL, cannot log metrics.")
  }
  
  
  # --- Prediction Loop ---
  # (Prediction loop requires the global predictor stacks, not species_specific_stack)
  predictions_made = 0; prediction_errors = 0
  # Use the original global predictor paths list for projection
  ...
  if (!is.null(final_model)) {
    for (pred_scenario in scenarios_to_predict) {
      ...
      # Check force_rerun flag from the appropriate group (standard or biotic)
      rerun_flag <- if (grepl("biotic|combined", predictor_type_suffix)) config$force_rerun$run_biotic_sdms else config$force_rerun$run_standard_sdms
      if (!rerun_flag && file.exists(target_pred_file_path)) {
        slog("DEBUG", "  Prediction exists in target location. Skipping."); next
      }
      
      # --- Load GLOBAL predictor stack for this scenario ---
      ...
      # Predict using the final model and the GLOBAL stack for the scenario
  ...
  # --- Clean up ---
  # Remove objects specific to this species run
  if (exists("species_specific_stack", inherits = FALSE)) rm(species_specific_stack)
  if (exists("tuning_predictor_stack", inherits = FALSE)) rm(tuning_predictor_stack) # This was the global one loaded initially
  ...
  if (exists("spatial_folds", inherits=FALSE)) rm(spatial_folds)
