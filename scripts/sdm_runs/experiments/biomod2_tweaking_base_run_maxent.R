# scripts/sdm_runs/experiments/biomod2_tweaking_base_run_maxent.R
#-------------------------------------------------------------------------------
# Tweaking script for a basic BIOMOD2 run with default/bigboss parameters.
# Focuses on running MAXENT for a single species.
# Placeholder for blockCV.
#-------------------------------------------------------------------------------

cat("--- Running Script biomod2_tweaking_base_run_maxent.R ---\n")
...
# --- 1.1. Maxent.jar Configuration (Specific to MAXENT) ---
# Path provided by user
config$path_to_maxent_jar <- "/home/bi-server-kyoto/R/x86_64-pc-linux-gnu-library/4.4/dismo/java/maxent.jar" 

if (!file.exists(config$path_to_maxent_jar)) {
  stop(paste("FATAL: maxent.jar not found at specified path:", config$path_to_maxent_jar,
             "\nPlease ensure dismo is installed and maxent.jar is available, or update the path."))
} else {
  cat(paste(Sys.time(), "[INFO] Using maxent.jar from:", config$path_to_maxent_jar, "\n"))
  # Set the option for biomod2 to find the maxent.jar
  # This allows BIOMOD_Modeling to find it without modifying the helper function's arguments.
  options(biomod2.MAXENT.MAXENT.jar.path = config$path_to_maxent_jar)
}
...
pacman::p_load(terra, sf, dplyr, readr, tools, stringr,
               biomod2, rJava, # Core biomod2 and rJava for MAXENT
...
cat(paste(Sys.time(), "[INFO] --- Starting Script biomod2_tweaking_base_run_maxent.R ---\n"))
...
process_species_biomod2_basic <- function(sp_name_actual, sp_name_biomod, sp_aphia,
  ...
  cat(log_prefix, "INFO [BIOMOD2_Process] --- Starting basic BIOMOD2 processing with MAXENT and blockCV ---\n")
  ...
  # 8.5 Run MAXENT with 'bigboss' options (or defaults) and the CV table
  ...
  myBiomodModelOutCurrent <- run_biomod2_models_basic( # This helper now uses the CV table
    myBiomodDataCurrent, myBiomodCVTableCurrent, 'MAXENT', # <<< CHANGED FROM MAXNET TO MAXENT
  ...
  algo_to_project_current <- 'MAXENT' # <<< CHANGED FROM MAXNET TO MAXENT
  ...
  # The suffix will automatically update due to algo_to_project_current change
  biomod2_model_run_suffix <- paste0(pred_type_suffix_base_val, "_biomod2_", algo_to_project_current, "_blockCV_default") 
  ...
  cat(log_prefix, "INFO [BIOMOD2_Process] --- Basic BIOMOD2 processing with MAXENT and blockCV finished ---\n")
  return(list(status = "success_biomod2_maxent_blockcv", species = sp_name_actual, biomodModel=myBiomodModelOutCurrent))
...
cat(paste(Sys.time(), "[INFO] --- Calling BIOMOD2 processing (MAXENT) for species:", species_name_actual, "---\n"))
biomod2_run_result <- process_species_biomod2_basic(
  ...
  cfg_obj = config # config now contains path_to_maxent_jar
)

# --- 10. Process Results (Very Simplified for this Tweaking Script) ---
cat(paste(Sys.time(), "[INFO]--- BIOMOD2 Tweaking Run (MAXENT) Result ---\n"))
if (!is.null(biomod2_run_result)) {
  ...
  if (!is.null(biomod2_run_result$biomodModel)) {
    cat("  BIOMOD_Modeling output object (MAXENT) generated.\n")
    # Example: Get evaluations if you want to see them
    # print(get_evaluations(biomod2_run_result$biomodModel))
  }
} else {
  cat(paste(Sys.time(), "[ERROR] BIOMOD2 processing (MAXENT) returned NULL overall.\n"))
}

gc(full=TRUE)
cat(paste(Sys.time(), "[INFO]--- Script biomod2_tweaking_base_run_maxent.R finished. ---\n"))
#-------------------------------------------------------------------------------