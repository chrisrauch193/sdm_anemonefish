...
#-------------------------------------------------------------------------------
# Run SDMs for Anemonefish Species using PC1 + Max Host Suitability (Ensemble)
...
# Includes Parallel Processing, Logging, Progress, Species-Specific Logs.
# Saves outputs to target structure. Assumes 06a produced host predictions.
# v7 - Ensemble Implementation.
#-------------------------------------------------------------------------------
cat("--- Running Script 06c: Run Anemonefish Biotic+PC1 Ensemble SDMs (v7) ---\n")
...
log4r::info(logger, "--- Starting Script 06c: Run Anemonefish Biotic+PC1 Ensemble SDMs (v7) ---")
...
for (scenario in scenarios_to_process) {
  ...
  if(length(all_host_prediction_paths[[scenario]]) == 0 && missing_files_scenario > 0){
    log4r::error(logger, paste("FATAL: No host predictions found for ANY species in target location for scenario:", scenario, ". Ensure 06a ran successfully."))
    missing_host_preds_overall <- TRUE
...
if(missing_host_preds_overall) stop("Halting script: required host prediction files missing for one or more scenarios. Check logs/06a output.")
...
# --- 11. Define Function to Process Single Species (Biotic + PC1 Ensemble v7) ---
process_species_sdm_biotic_pc1 <- function(species_row, config, env_predictor_paths_list, all_host_prediction_paths, associations_df,
  ...
  slog("INFO", "--- Starting Biotic+PC1 ENSEMBLE processing (v7) ---")
  ...
  final_model_list_file <- file.path(intermediate_models_dir, paste0("sdm_model_", species_name_sanitized, predictor_type_suffix, ".rds")) # Expects a list
  ...
  # --- Prepare PC1+Biotic Predictor Stack for Tuning Scenario ---
  slog("DEBUG", "Preparing PC1+Biotic predictor stack for tuning scenario:", tuning_scenario)
  tuning_predictor_stack <- NULL # Initialize
  tryCatch({
    tuning_env_path <- env_predictor_paths_list[[tuning_scenario]]
    if (is.null(tuning_env_path) || !file.exists(tuning_env_path)) stop("Env PCA stack path missing for tuning.")
    pc1_tuning_layer <- terra::rast(tuning_env_path)[[1]] # Select only PC1
    ...
    if (length(host_paths_for_tuning) == 0) stop("No host prediction paths found for tuning.")
    ...
    if (length(host_rasters_tuning_list) == 0) stop("Failed load/resample ANY hosts for tuning.")
    ...
    tuning_predictor_stack <- c(pc1_tuning_layer, max_host_suitability_tuning) # Combine PC1 and Biotic
    names(tuning_predictor_stack) <- c("PC1", "host_suitability_max") # Ensure names are correct
    slog("INFO", paste("Tuning predictor stack created:", paste(names(tuning_predictor_stack), collapse=", ")))
    rm(pc1_tuning_layer, host_stack_tuning, max_host_suitability_tuning, host_rasters_tuning_list); gc()
  }, error = function(e) {
    msg <- paste0("Skipping: Failed prepare PC1+Biotic tuning stack: ", e$message); slog("ERROR", msg)
    # Cleanup partial objects
    if(exists("pc1_tuning_layer")) rm(pc1_tuning_layer); if(exists("host_stack_tuning")) rm(host_stack_tuning); if(exists("max_host_suitability_tuning")) rm(max_host_suitability_tuning); if(exists("host_rasters_tuning_list")) rm(host_rasters_tuning_list); gc()
    return(list(status = "error_tuning_predictors", species = species_name, occurrence_count = NA, message = msg))
  })
  if (is.null(tuning_predictor_stack)) { return(list(status = "error_tuning_predictors", species = species_name, occurrence_count = NA, message = paste0("Failed create tuning predictor stack."))) }
  # --- End Stack Prep ---
  
  # --- Load/Clean Occurrences ---
  config_for_occ_load <- config; config_for_occ_load$predictor_stack_for_thinning <- tuning_predictor_stack
  slog("DEBUG", "Loading/cleaning/thinning occurrences using PC1+Biotic tuning stack.")
  ...
  # --- Tuning & Training ---
  final_model_list <- NULL; tuning_output <- NULL; load_existing_models <- FALSE; full_swd_data <- NULL
  
  if (!config$force_rerun$run_biotic_sdms && file.exists(final_model_list_file)) { # Check biotic flag
    ...
    if(!is.null(final_model_list) && is.list(final_model_list) && length(final_model_list) > 0 && all(sapply(final_model_list, inherits, "SDMmodel")) && !is.null(names(final_model_list))) {
      ...
      if(!is.null(background_points_for_vi)){ full_swd_data <- tryCatch({ SDMtune::prepareSWD(species = species_name, p = occs_coords, a = background_points_for_vi, env = tuning_predictor_stack, verbose = FALSE) }, error = function(e) { slog("WARN", "Failed prepareSWD for VI on loaded model list:", e$message); NULL }) } else { slog("WARN", "Failed background gen for VI on loaded model list.") }
      rm(background_points_for_vi); gc()
    } else { msg <- paste0("Existing final model list file invalid/failed load. Will re-run."); slog("WARN", msg); load_existing_models <- FALSE; final_model_list <- NULL }
  }
  ...
  if (!load_existing_models) {
    ...
    if(!save_tuning_results(tuning_output, species_name_sanitized, predictor_type_suffix, config, logger=NULL, species_log_file=species_log_file)) { slog("WARN", "Failed to save tuning results.") } # Don't stop on failure
    ...
    if(!save_final_model(final_model_list, species_name_sanitized, predictor_type_suffix, config, logger=NULL, species_log_file=species_log_file)) { slog("WARN", "Failed to save final model list.") } # Don't stop on failure
  ...
  if (!is.null(final_model_list)) {
    if(!is.null(full_swd_data)){
      ...
      vi_success <- calculate_and_save_vi(
        final_model_list = final_model_list, training_swd = full_swd_data,
        species_name_sanitized = species_name_sanitized, group_name = group_name,
        predictor_type_suffix = predictor_type_suffix, config = config,
        logger = NULL, species_log_file = species_log_file
      )
  ...
  } else { slog("WARN", "Skipping variable importance: final model list unavailable.") }
  ...
  if (!is.null(final_model_list)) {
    for (pred_scenario in scenarios_to_predict) {
      ...
      prediction_predictor_stack <- NULL # Initialize
      tryCatch({
        ...
        pc1_pred_layer <- terra::rast(pred_env_path)[[1]] # Select PC1
        ...
        prediction_predictor_stack <- c(pc1_pred_layer, max_host_suitability_scenario) # Combine PC1 + Biotic
        names(prediction_predictor_stack) <- c("PC1", "host_suitability_max") # Ensure correct names
        slog("DEBUG", paste("  Predictor stack for prediction:", paste(names(prediction_predictor_stack), collapse=", ")))
        rm(pc1_pred_layer, host_stack_scenario, max_host_suitability_scenario, host_rasters_pred_list); gc()
        
      }, error = function(e) {
        slog("ERROR", paste("  Failed preparing predictor stack for prediction:", pred_scenario, "Error:", e$message))
        if (exists("pc1_pred_layer")) rm(pc1_pred_layer); if (exists("host_stack_scenario")) rm(host_stack_scenario); if (exists("max_host_suitability_scenario")) rm(max_host_suitability_scenario); if (exists("host_rasters_pred_list")) rm(host_rasters_pred_list); gc()
        prediction_predictor_stack <- NULL
      })
      # --- End Stack Prep ---
      
      if(is.null(prediction_predictor_stack)) { prediction_errors <- prediction_errors + 1; next }
      
      # Predict & Save (using the ENSEMBLE prediction helper)
      prediction_output <- predict_sdm_suitability(final_model_list, prediction_predictor_stack, config, logger=NULL, species_log_file=species_log_file)
      ...
      rm(prediction_predictor_stack); gc()
    } # End prediction scenario loop
  } else {
    prediction_errors <- length(scenarios_to_predict); msg <- paste0("Skipping predictions: Final model list unavailable."); slog("ERROR", msg)
  ...
  final_status <- "success"; status_message <- paste0("Finished Biotic+PC1 Ensemble SDM. Occs:", occurrence_count_after_thinning, ". Preds attempted:", length(scenarios_to_predict), ". Made:", predictions_made, ". Errors/Skipped:", prediction_errors, ".")
...
log4r::info(logger, paste("Starting Biotic+PC1 Ensemble SDM processing for", nrow(species_df), group_name, "species..."))
...
log4r::info(logger, "--- Script 06c (v7 - Ensemble Implementation) finished. ---")
#-------------------------------------------------------------------------------