# scripts/config.R
#-------------------------------------------------------------------------------
# Configuration Settings for Anemone/Anemonefish SDM Project (v7 - ENMeval Fix)
#-------------------------------------------------------------------------------
...
pacman::p_load(here, dplyr, terra, sf, stringr, ggplot2, readr, readxl,
               ...
               # SDMtune, # SDMtune might still be needed for some helper functions
               dismo, raster, # Dismo/raster might be needed by dependencies
               ENMeval, # <--- Added ENMeval
               predicts, # <--- Added predicts (used by ENMeval internally)
...
# ... (keep this section as is) ...
apply_indo_pacific_crop <- TRUE
indo_pacific_bbox <- c(xmin=30, xmax=180, ymin=-50, ymax=50)
mask_background_points_to_coral <- TRUE

# --- Intermediate Output Paths ---
# ... (keep this section as is) ...
sdm_output_dir_intermediate   <- file.path(data_dir, "sdm_output_intermediate")
...
target_results_base <- file.path(base_dir, "predictions") # For CV CSVs (e.g., ENMeval results)
...
enmeval_scripts_dir <- file.path(scripts_dir, "ENMeval") # Directory for ENMeval scripts
helpers_dir      <- file.path(scripts_dir, "helpers")
dir.create(enmeval_scripts_dir, recursive = TRUE, showWarnings = FALSE) # Create the dir
...
force_rerun <- list(
  ...
  run_standard_sdms = TRUE, # Flag for original SDMtune runs (06a-d)
  run_enmeval_sdms = TRUE,  # <--- New flag for ENMeval runs
  run_biotic_sdms = TRUE # Keep this flag for biotic SDMtune runs (06c, 06d)
)

occurrence_crs  <- "EPSG:4326"
env_crs         <- "EPSG:4326"

# --- Original Download Lists --- (Keep as is)
# ... (keep this section as is) ...
...
# Added specific suffix for ENMeval outputs
model_output_subdir_map <- list(
  `_pca` = "",
  `_vif` = "vif", # Added example for VIF runs if needed
  `_biotic_pc1` = "biotic_pc1",
  `_combined_pca` = "combined_pca",
  `_enmeval_pca` = "enmeval_pca", # Suffix for ENMeval PCA run
  `_enmeval_vif` = "enmeval_vif"  # Suffix for ENMeval VIF run
)
...
# Paths for Intermediate PCA/VIF Results
# ... (keep this section as is) ...
...
# --- SDM Settings (Split for SDMtune vs ENMeval) ---

# --- SDMtune Specific Settings (Original 06a-d) ---
sdm_method <- "Maxnet" # Still needed for original scripts if run
# sdm_partitions <- "randomkfold" # Defined below for ENMeval context
# sdm_n_folds <- 5 # Defined below for ENMeval context
sdm_tune_grid <- list(reg = seq(0.5, 4, 0.5), fc = c("L", "LQ", "LH", "LQP", "LQH", "LQHP", "LQHPT")) # Use expanded grid for SDMtune? Or original? Keep expanded for now.
# sdm_evaluation_metric <- "auc" # Defined below for ENMeval context

# --- General Settings (Used by both or data prep) ---
# ... (keep this section as is) ...
pca_background_points_n <- 100000 # For PCA model building
background_points_n <- 10000      # For SDM background points
thinning_method <- "cell"         # Thinning method for occurrences ("none", "cell")
apply_coral_mask <- TRUE          # Used in data prep/bg point sampling
apply_depth_filter <- TRUE; depth_min <- -50; depth_max <- 0 # Used in data prep
min_occurrences_sdm <- 15

# --- ENMeval Specific Settings ---
enmeval_algorithms <- c("maxnet")
enmeval_partitions <- "checkerboard1" # <--- Set to checkerboard1
# Settings specific to the chosen partition method (passed as direct arguments to ENMevaluate)
enmeval_kfolds <- 5                    # Used if partitions = "randomkfold"
enmeval_aggregation_factor <- 25       # Used if partitions = "checkerboard1" or "checkerboard2"
enmeval_orientation <- "lat_lon"       # Used if partitions = "block"
# ... rest of ENMeval settings ...
enmeval_tuning_settings <- list( # Define the tuning parameters grid
    fc = c("L", "LQ", "H", "LQH", "LQHP", "LQHPT"),
    rm = seq(0.5, 4, 0.5)
)
enmeval_num_cores <- 1
enmeval_pred_type <- "cloglog"
enmeval_clamp <- TRUE
enmeval_selection_metric <- "AICc"
enmeval_delta_aicc_threshold <- 2
enmeval_omit_na_models_from_selection <- TRUE
...
config <- list(
  ...
  scripts_dir = scripts_dir, enmeval_scripts_dir = enmeval_scripts_dir, helpers_dir = helpers_dir,
  # Input Paths
  ...
  # Intermediate Output Paths
  sdm_output_dir_intermediate = sdm_output_dir_intermediate, log_dir_base = log_dir_base,
  species_log_dir = species_log_dir, models_dir_intermediate = models_dir_intermediate,
  results_dir_intermediate = results_dir_intermediate,
  # Target Output Paths
  ...
  target_results_base = target_results_base, target_vi_base = target_vi_base,
  # Scenario/Model Mapping
  env_scenarios = env_scenarios, scenario_folder_map = scenario_folder_map,
  ssp_scenario_map = ssp_scenario_map, model_output_subdir_map = model_output_subdir_map,
  # Processing Parameters
  ...
  # --- General SDM Settings ---
  pca_background_points_n = pca_background_points_n, background_points_n = background_points_n,
  thinning_method = thinning_method, apply_coral_mask = apply_coral_mask,
  apply_indo_pacific_crop = apply_indo_pacific_crop, indo_pacific_bbox = indo_pacific_bbox,
  mask_background_points_to_coral = mask_background_points_to_coral,
  apply_depth_filter = apply_depth_filter, depth_min = depth_min, depth_max = depth_max,
  min_occurrences_sdm = min_occurrences_sdm,
  # --- SDMtune Specific Settings (Keep for original scripts) ---
  ...
  # Partition arguments (add others if needed)
  enmeval_kfolds = enmeval_kfolds,
  enmeval_aggregation_factor = enmeval_aggregation_factor,
  enmeval_orientation = enmeval_orientation,
  # General ENMeval settings
