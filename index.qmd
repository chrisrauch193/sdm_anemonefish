...
pacman::p_load(here, dplyr, terra, sf, stringr, ggplot2, readr, tools, naturalearth, tidyterra, lwgeom)
...
construct_mean_prediction_filename <- function(species_name_sanitized, scenario_name, predictor_type_suffix, config) {
  ...
  base_filename <- paste0("2-pred_", species_name_sanitized) # Using SDMtune mean convention implicitly
  target_dir <- NULL
...
# TODO: NEW CODE
# --- Load MEOW ecoregions once (for native/reachable polygons later) ---

sf::sf_use_s2(FALSE)

meow_path <- file.path(config$base_dir, "data", "shapefiles", "meow_ecos.shp")
if (!file.exists(meow_path)) stop("MEOW shapefile not found: ", meow_path)

# Keep your old variable name to avoid breaking other code if you already reference it
marine_realms_sf <- sf::st_read(meow_path, quiet = TRUE)

# ---- helper: make_valid that works across sf/lwgeom versions ----
make_valid <- function(x) {
  if ("st_make_valid" %in% getNamespaceExports("sf")) {
    return(sf::st_make_valid(x))
  }
  if (requireNamespace("lwgeom", quietly = TRUE) &&
      "lwgeom_make_valid" %in% getNamespaceExports("lwgeom")) {
    return(lwgeom::lwgeom_make_valid(x))
  }
  stop("No st_make_valid available: please update sf (preferred) or lwgeom.")
}

# ---- helper: wrap_dateline if available; otherwise skip ----
wrap_dateline_if_available <- function(x) {
  if ("st_wrap_dateline" %in% getNamespaceExports("sf")) {
    return(sf::st_wrap_dateline(
      x,
  ...
  if (requireNamespace("lwgeom", quietly = TRUE) &&
      "st_wrap_dateline" %in% getNamespaceExports("lwgeom")) {
    return(lwgeom::st_wrap_dateline(
      x,
      options = c("WRAPDATELINE=YES", "DATELINEOFFSET=180"),
      quiet = TRUE
    ))
  }
  message("ℹ️ st_wrap_dateline() not available in your sf/lwgeom; skipping dateline wrap.")
  x
}

# Basic validity + zero-width buffer (cheap first pass)
marine_realms_sf <- make_valid(marine_realms_sf)
marine_realms_sf <- suppressWarnings(sf::st_buffer(marine_realms_sf, 0))
marine_realms_sf <- marine_realms_sf[!sf::st_is_empty(marine_realms_sf), ]
cat("Loaded MEOW polygons (raw rows):", nrow(marine_realms_sf), "\n")

# Dissolve to a single geometry per ecoregion ID (so selection is “whole ecoregions”)
meow_id_candidates <- c("ECOREGION", "ECO_CODE", "ECO_ID", "ECO_NAME")
meow_id_col <- meow_id_candidates[meow_id_candidates %in% names(marine_realms_sf)][1]
if (is.na(meow_id_col)) {
  stop("Could not find an ecoregion ID column in meow_ecos.shp. Columns are: ",
       paste(names(marine_realms_sf), collapse = ", "))
}

meow_ecos_sf <- marine_realms_sf %>%
  dplyr::mutate(.meow_id = .data[[meow_id_col]]) %>%
  dplyr::group_by(.meow_id) %>%
  dplyr::summarise(geometry = sf::st_union(geometry), .groups = "drop")

# Dateline wrap only if your install supports it (otherwise skip)
meow_ecos_sf <- wrap_dateline_if_available(meow_ecos_sf)

# Repair + keep polygonal types only
meow_ecos_sf <- make_valid(meow_ecos_sf)
meow_ecos_sf <- sf::st_collection_extract(meow_ecos_sf, "POLYGON")
meow_ecos_sf <- sf::st_cast(meow_ecos_sf, "MULTIPOLYGON", warn = FALSE)

# Final heal
meow_ecos_sf <- suppressWarnings(sf::st_buffer(meow_ecos_sf, 0))
meow_ecos_sf <- meow_ecos_sf[!sf::st_is_empty(meow_ecos_sf), ]

cat("MEOW dissolved ecoregions:", nrow(meow_ecos_sf), " (ID col:", meow_id_col, ")\n")

# Precompute adjacency (“neighbors”) ONCE for speed (touching polygons)
# IMPORTANT: use s2 for robust global topology (don’t project to 3857 here)
old_s2 <- sf::sf_use_s2()
sf::sf_use_s2(TRUE)

meow_neighbors <- tryCatch(
  sf::st_touches(meow_ecos_sf),
  error = function(e) {
    message("⚠️ st_touches failed; neighbor expansion will be disabled. Error: ", e$message)
    NULL
  }
)
...
if (!is.null(meow_neighbors)) {
  cat("Computed MEOW adjacency list for", length(meow_neighbors), "ecoregions.\n")
}
# TODO: NEW CODE
...
plot_prediction_map <- function(prediction_raster, species_name, world_basemap,
                                occurrence_points_sf = NULL,
                                native_poly_sf = NULL,
                                reachable_poly_sf = NULL,
                                mask_poly_sf = NULL,          # NEW
                                max_display_depth = -750,
                                display_bbox = c(xmin = 30, xmax = 180, ymin = -31, ymax = 35)) {

  # --- Mask 1: crop to display bbox (fast) ---
  prediction_raster <- terra::crop(prediction_raster, terra::ext(display_bbox))

  # --- Mask 2: bathymetry cutoff (align bathy to raster grid) ---
  bathy_aligned <- terra::project(global_bathymetry, prediction_raster, method = "average")
  prediction_raster <- terra::ifel(is.na(bathy_aligned) | bathy_aligned < max_display_depth, NA, prediction_raster)

  # --- Mask 3: optional polygon mask (native or reachable) ---
  if (!is.null(mask_poly_sf)) {
    # accept sf or sfc
    if (inherits(mask_poly_sf, "sfc")) mask_poly_sf <- sf::st_as_sf(mask_poly_sf)
    if (!inherits(mask_poly_sf, "sf")) stop("mask_poly_sf must be sf or sfc")

    # transform polygon to raster CRS
    poly_t <- sf::st_transform(mask_poly_sf, sf::st_crs(terra::crs(prediction_raster)))
    poly_v <- terra::vect(poly_t)

    # mask raster to polygon footprint
    prediction_raster <- terra::mask(prediction_raster, poly_v, touches = TRUE)
  }

  names(prediction_raster) <- "Suitability"

  custom_color_scale <- scale_fill_gradientn(
    colors = c("#FEF9C3", "#FDBA74", "#D946EF", "#4F46E5"),
    na.value = "transparent",
    limits = c(0, 1),
    name = "Occurrence\nprobability"
  )

  # Transform overlays to raster CRS (avoids sf warnings / misalignment)
  raster_crs <- sf::st_crs(terra::crs(prediction_raster))
  if (!is.null(occurrence_points_sf) && inherits(occurrence_points_sf, "sf")) {
    occurrence_points_sf <- sf::st_transform(occurrence_points_sf, raster_crs)
  }
  if (!is.null(native_poly_sf) && inherits(native_poly_sf, "sfc")) native_poly_sf <- sf::st_as_sf(native_poly_sf)
  if (!is.null(reachable_poly_sf) && inherits(reachable_poly_sf, "sfc")) reachable_poly_sf <- sf::st_as_sf(reachable_poly_sf)
  if (!is.null(native_poly_sf) && inherits(native_poly_sf, "sf")) native_poly_sf <- sf::st_transform(native_poly_sf, raster_crs)
  if (!is.null(reachable_poly_sf) && inherits(reachable_poly_sf, "sf")) reachable_poly_sf <- sf::st_transform(reachable_poly_sf, raster_crs)
  ...
  # overlays (optional; keep for context)
  if (!is.null(reachable_poly_sf) && inherits(reachable_poly_sf, "sf")) {
    p <- p + geom_sf(data = reachable_poly_sf, fill = NA, linewidth = 0.4, linetype = "dashed")
  }
  if (!is.null(native_poly_sf) && inherits(native_poly_sf, "sf")) {
    p <- p + geom_sf(data = native_poly_sf, fill = NA, linewidth = 0.5)
  }

  if (!is.null(occurrence_points_sf) && inherits(occurrence_points_sf, "sf") && nrow(occurrence_points_sf) > 0) {
    p <- p + geom_sf(
      data = occurrence_points_sf,
      color = "black",
      fill = "yellow",
      shape = 21,
      size = 1.5,
  ...
  p +
    coord_sf(crs = raster_crs,
             xlim = terra::ext(prediction_raster)[c(1,2)],
             ylim = terra::ext(prediction_raster)[c(3,4)],
             expand = FALSE) +
...
# TODO: NEW CODE
# --- 3b) Define MEOW-native (M0) + reachable (M1) polygons per species (NO hard raster masking) ---

# Turn this on/off easily for map overlays
use_meow_polygons_on_maps <- TRUE

# Small cache so repeated plotting is fast
.meow_poly_cache <- new.env(parent = emptyenv())

read_occ_sf <- function(occ_file) {
  if (!file.exists(occ_file)) return(NULL)
  occ_df <- readr::read_csv(occ_file, show_col_types = FALSE) %>%
    dplyr::filter(!is.na(decimalLongitude), !is.na(decimalLatitude))
  if (nrow(occ_df) == 0) return(NULL)
  sf::st_as_sf(occ_df, coords = c("decimalLongitude", "decimalLatitude"), crs = 4326, remove = FALSE)
}

drop_far_outliers_simple <- function(pts_sf) {
  if (is.null(pts_sf) || nrow(pts_sf) < 10) return(pts_sf)
...
expand_neighbors <- function(seed_idx, neighbors_list, steps = 1) {
  idx <- unique(seed_idx)
  if (steps <= 0) return(idx)
  for (k in seq_len(steps)) {
    neigh <- unique(unlist(neighbors_list[idx]))
    idx <- unique(c(idx, neigh))
  }
  idx
}

make_native_poly_meow <- function(occ_sf, meow_ecos_sf, template_r,
                                  keep_frac = 0.95, min_pts_ecoregion = 2,
                                  neighbor_steps = 0) {
  if (is.null(occ_sf) || nrow(occ_sf) == 0) return(NULL)

  crs_t <- sf::st_crs(terra::crs(template_r))
  occ_t  <- sf::st_transform(occ_sf,  crs_t)
  ecos_t <- sf::st_transform(meow_ecos_sf, crs_t)

  occ_t <- drop_far_outliers_simple(occ_t)

  hits <- sf::st_intersects(occ_t, ecos_t)
  idx <- unlist(hits)
  if (length(idx) == 0) return(NULL)

  counts <- sort(table(idx), decreasing = TRUE)
  ...
  keep_by_min <- as.integer(names(counts)[counts >= min_pts_ecoregion])

  keep_idx <- unique(c(keep_by_frac, keep_by_min))

  # Expand to neighbors using the precomputed adjacency list
  if (neighbor_steps > 0) {
    keep_idx <- expand_neighbors(keep_idx, meow_neighbors, steps = neighbor_steps)
  }

  poly <- sf::st_union(ecos_t[keep_idx, , drop = FALSE])
  poly <- sf::st_make_valid(poly)
  poly <- suppressWarnings(sf::st_buffer(poly, 0))
  poly
}

get_species_meow_polys <- function(aphia_id, occ_dir, template_r,
                                   keep_frac = 0.95, min_pts_ecoregion = 2,
                                   neighbor_steps_reachable = 1) {
  if (is.null(aphia_id) || length(aphia_id) != 1 || is.na(aphia_id)) return(list(native = NULL, reachable = NULL))

  cache_key <- paste0(aphia_id, "|", terra::crs(template_r), "|", neighbor_steps_reachable)
  if (exists(cache_key, envir = .meow_poly_cache, inherits = FALSE)) {
    return(get(cache_key, envir = .meow_poly_cache, inherits = FALSE))
  }

  occ_file <- file.path(occ_dir, paste0(aphia_id, ".csv"))
  occ_sf <- read_occ_sf(occ_file)

  native <- make_native_poly_meow(
    occ_sf, meow_ecos_sf, template_r,
    keep_frac = keep_frac, min_pts_ecoregion = min_pts_ecoregion,
    neighbor_steps = 0
  )

  reachable <- make_native_poly_meow(
    occ_sf, meow_ecos_sf, template_r,
    keep_frac = keep_frac, min_pts_ecoregion = min_pts_ecoregion,
    neighbor_steps = neighbor_steps_reachable
  )

  out <- list(native = native, reachable = reachable)
  assign(cache_key, out, envir = .meow_poly_cache)
  out
}

cat("✅ MEOW polygon helpers ready (native=M0, reachable=M1). No hard raster masking applied.\n")
# TODO: NEW CODE
...
# Put richness maps in their own folder
richness_dir <- file.path(figure_output_dir, "community_richness")
dir.create(richness_dir, recursive = TRUE, showWarnings = FALSE)

# helper: consistent save settings
save_plot_png <- function(p, out_path, width = 10, height = 7, dpi = 300) {
...
if (!is.null(host_richness_sum_cropped)) {
  ...
  save_plot_png(host_richness_plot, host_plot_filename)
...
if (!is.null(fish_richness_sum_cropped)) {
  ...
  save_plot_png(fish_richness_plot, fish_plot_filename)
...
cat("\n--- Saving Individual Species Suitability Maps (3 versions each) ---\n")

# Directory tree for species maps
species_root <- file.path(figure_output_dir, "species_maps")

host_full_dir      <- file.path(species_root, "hosts", "full")
host_native_dir    <- file.path(species_root, "hosts", "native_masked")
host_reachable_dir <- file.path(species_root, "hosts", "reachable_masked")

fish_full_dir      <- file.path(species_root, "fish", "full")
fish_native_dir    <- file.path(species_root, "fish", "native_masked")
fish_reachable_dir <- file.path(species_root, "fish", "reachable_masked")

dir.create(host_full_dir,      recursive = TRUE, showWarnings = FALSE)
dir.create(host_native_dir,    recursive = TRUE, showWarnings = FALSE)
dir.create(host_reachable_dir, recursive = TRUE, showWarnings = FALSE)
dir.create(fish_full_dir,      recursive = TRUE, showWarnings = FALSE)
dir.create(fish_native_dir,    recursive = TRUE, showWarnings = FALSE)
dir.create(fish_reachable_dir, recursive = TRUE, showWarnings = FALSE)
...
if (!is.null(host_pred_stack_cropped)) {
  ...
  for(i in 1:nlyr(host_pred_stack_cropped)) {
    ...
    if (exists("use_meow_polygons_on_maps") && isTRUE(use_meow_polygons_on_maps) && length(aphia_id) == 1) {
      polys <- get_species_meow_polys(
        aphia_id = aphia_id,
        occ_dir = config$anemone_occurrence_dir,
        template_r = species_raster,
        keep_frac = 0.95,
        min_pts_ecoregion = 2,
        neighbor_steps_reachable = 1
      )
      native_poly <- polys$native
      reachable_poly <- polys$reachable
    }
    
    p_full <- plot_prediction_map(
      prediction_raster = species_raster,
      species_name = gsub("_", " ", sp_name),
      world_basemap = world_map_sf,
      occurrence_points_sf = occ_sf,
      native_poly_sf = native_poly,
      reachable_poly_sf = reachable_poly,
      max_display_depth = -750
    )
    save_plot_png(p_full, file.path(host_full_dir, out_base))
    # print(p)
    
    
    p_native <- plot_prediction_map(
      prediction_raster = species_raster,
      species_name = paste0(gsub("_", " ", sp_name), " (native masked)"),
      world_basemap = world_map_sf,
      occurrence_points_sf = occ_sf,
      native_poly_sf = NULL,
      reachable_poly_sf = reachable_poly,
      mask_poly_sf = native_poly,      # HERE
      max_display_depth = -750
    )
    save_plot_png(p_native, file.path(host_native_dir, out_base))
    # print(p_native)
    
    p_reachable <- plot_prediction_map(
      prediction_raster = species_raster,
      species_name = paste0(gsub("_", " ", sp_name), " (reachable masked)"),
      world_basemap = world_map_sf,
      occurrence_points_sf = occ_sf,
      native_poly_sf = NULL,
      reachable_poly_sf = NULL,
      mask_poly_sf = reachable_poly,   # HERE
      max_display_depth = -750
    )
    save_plot_png(p_reachable, file.path(host_reachable_dir, out_base))
    # print(p_reachable)
...
if (!is.null(fish_pred_stack_cropped)) {
  ...
  for(i in 1:nlyr(fish_pred_stack_cropped)) {
    ...
    save_plot_png(p_full, file.path(fish_full_dir, out_base))
    # print(p)
    ...
    save_plot_png(p_native, file.path(fish_native_dir, out_base))
    # print(p_native)
    ...
    save_plot_png(p_reachable, file.path(fish_reachable_dir, out_base))
    # print(p_reachable)
    # TODO: NEW CODE
  }
}
...
# NOTE: This chunk no longer builds fish_occ_all_sf.
# It prepares MEOW + depth masking inputs in a way that matches your top-of-page plotting.
...
# -----------------------------
# 0) Sanity checks: objects created earlier in the QMD
# -----------------------------
need <- c(
  "fish_pred_stack_cropped",
  "host_pred_stack_cropped",
  "anemonefish_species_df",
  "anemone_species_df",
  "get_species_meow_polys",
  "global_bathymetry"
)
...
# -----------------------------
# 1) Mask controls (these are the only knobs you should touch)
# -----------------------------
use_meow <- TRUE

# Which MEOW polygon should become the MASK?
# - "native"    -> M0 (solid outline in your suitability maps)
# - "reachable" -> M1 (dashed outline in your suitability maps)
conservation_mask_mode <- "reachable"

# How many MEOW neighbor-expansion steps define "reachable"
neighbor_steps_reachable <- 1
if (!exists("meow_neighbors") || is.null(meow_neighbors)) {
  message("⚠️ meow_neighbors is missing/NULL -> forcing neighbor_steps_reachable = 0 (no expansion).")
  neighbor_steps_reachable <- 0
}

# Should the conservation mask be built from fish only, or fish+hosts combined?
conservation_mask_taxa <- "both"  # "fish" or "both"

# Depth cutoff (SAME as plot_prediction_map default)
max_display_depth_conservation <- -750

# Optional: crop final conservation figures to match your other maps
# (this is purely for consistent figure framing, not ecological masking)
conservation_display_bbox <- c(xmin = 30, xmax = 180, ymin = -31, ymax = 35)

# -----------------------------
# 2) Build AphiaID lists that match the species present in the stacks
# -----------------------------
fish_species_space <- gsub("_", " ", names(fish_pred_stack_cropped))
fish_aphia_ids <- anemonefish_species_df$AphiaID[match(fish_species_space, anemonefish_species_df$scientificName)]

host_species_space <- gsub("_", " ", names(host_pred_stack_cropped))
host_aphia_ids <- anemone_species_df$AphiaID[match(host_species_space, anemone_species_df$scientificName)]

cat("Fish stack species:", length(fish_species_space), " | AphiaIDs matched:", sum(!is.na(fish_aphia_ids)), "\n")
cat("Host stack species:", length(host_species_space), " | AphiaIDs matched:", sum(!is.na(host_aphia_ids)), "\n")

# -----------------------------
# 3) Robust MEOW union helpers (fixes sfg/sfc union errors)
# -----------------------------
as_sfc <- function(x, crs) {
  if (is.null(x)) return(NULL)
  if (inherits(x, "sf")) x <- sf::st_geometry(x)
  if (inherits(x, "sfg")) x <- sf::st_sfc(x, crs = crs)
  if (!inherits(x, "sfc")) stop("Expected sf/sfc/sfg geometry; got: ", paste(class(x), collapse = ", "))
  if (is.na(sf::st_crs(x))) sf::st_crs(x) <- crs
  x
}

build_union_meow_poly <- function(aphia_ids, occ_dir, template_r,
                                 mode = c("native", "reachable"),
                                 neighbor_steps = 1) {
  mode <- match.arg(mode)
  crs_t <- sf::st_crs(terra::crs(template_r))

  aphia_ids <- aphia_ids[!is.na(aphia_ids)]
  if (length(aphia_ids) == 0) return(NULL)

  polys <- lapply(aphia_ids, function(id) {
    p <- get_species_meow_polys(
      aphia_id = id,
      occ_dir = occ_dir,
      template_r = template_r,
      neighbor_steps_reachable = neighbor_steps
    )
    if (mode == "native") p$native else p$reachable
  })

  polys <- lapply(polys, as_sfc, crs = crs_t)
  polys <- polys[!vapply(polys, is.null, logical(1))]
  if (length(polys) == 0) return(NULL)

  poly_all <- do.call(c, polys)       # concatenate sfc vectors
  poly <- sf::st_union(poly_all)      # unary union -> sfc length 1
  poly <- sf::st_make_valid(poly)
  poly <- suppressWarnings(sf::st_buffer(poly, 0))
  poly
}

get_conservation_union_meow_poly <- function(template_r) {
  if (!isTRUE(use_meow)) return(NULL)

  if (conservation_mask_taxa == "fish") {
    return(build_union_meow_poly(
      fish_aphia_ids, config$anemonefish_occurrence_dir, template_r,
      mode = conservation_mask_mode,
      neighbor_steps = neighbor_steps_reachable
    ))
  }

  poly_fish <- build_union_meow_poly(
    fish_aphia_ids, config$anemonefish_occurrence_dir, template_r,
    mode = conservation_mask_mode,
    neighbor_steps = neighbor_steps_reachable
  )
  poly_host <- build_union_meow_poly(
    host_aphia_ids, config$anemone_occurrence_dir, template_r,
    mode = conservation_mask_mode,
    neighbor_steps = neighbor_steps_reachable
  )

  crs_t <- sf::st_crs(terra::crs(template_r))
  poly_fish <- as_sfc(poly_fish, crs_t)
  poly_host <- as_sfc(poly_host, crs_t)

  if (!is.null(poly_fish) && !is.null(poly_host)) {
    return(sf::st_union(c(poly_fish, poly_host)))
  }
  poly_fish %||% poly_host
}

cat("✅ Conservation mask setup ready. (mode=", conservation_mask_mode,
    ", taxa=", conservation_mask_taxa,
    ", max_display_depth=", max_display_depth_conservation, ")\n", sep = "")
...
#| message: false
#| warning: false
#| fig-cap: "Masking for conservation plots (MEOW + depth cutoff only)"

if (!require("pacman")) install.packages("pacman")
pacman::p_load(terra, sf, dplyr)

cat("\n--- Masking (MEOW + depth cutoff): starting ---\n")
...
need <- c(
  ...
  "global_bathymetry",
  "get_conservation_union_meow_poly",
  "max_display_depth_conservation"
)
missing <- need[!vapply(need, exists, logical(1))]
if (length(missing) > 0) stop("Missing required objects: ", paste(missing, collapse = ", "))
...
safe_align_to <- function(r, ref, method = "bilinear") {
  ...
  if (terra::crs(r) != terra::crs(ref)) r <- terra::project(r, ref)
  same_geom <- terra::compareGeom(r, ref, stopOnError = FALSE, crs = TRUE, ext = TRUE, res = TRUE, rowcol = TRUE)
...
# -----------------------------
# 1) Depth mask (MATCHES plot_prediction_map)
# keep cells where bathy >= max_display_depth_conservation
# -----------------------------
cat("Masking: building depth mask with max_display_depth = ", max_display_depth_conservation, "\n", sep = "")

bathy_aligned <- terra::project(global_bathymetry, template_r, method = "average")
depth_mask <- terra::ifel(is.na(bathy_aligned) | bathy_aligned < max_display_depth_conservation, NA, 1)
depth_mask <- as_keep_mask(depth_mask, template_r)

# -----------------------------
# 2) MEOW union mask (native or reachable, fish or both)
# -----------------------------
meow_mask <- NULL
union_poly_meow <- get_conservation_union_meow_poly(template_r)

if (!is.null(union_poly_meow)) {
  cat("Masking: building MEOW union mask\n")
  union_poly_meow_sf <- sf::st_as_sf(union_poly_meow)
  union_poly_meow_sf <- sf::st_transform(union_poly_meow_sf, sf::st_crs(terra::crs(template_r)))

  union_v <- terra::vect(union_poly_meow_sf)
  meow_mask <- terra::rasterize(union_v, template_r, field = 1, background = NA, touches = TRUE)
  meow_mask <- as_keep_mask(meow_mask, template_r)
} else {
  cat("Masking: MEOW union poly is NULL -> skipping MEOW mask (depth-only).\n")
}

# -----------------------------
# 3) Final analysis mask = depth AND meow
# -----------------------------
analysis_mask <- and_masks(depth_mask, meow_mask)

if (is.null(analysis_mask)) {
  cat("Masking: no mask constructed -> leaving stacks unmasked\n")
} else {
  cat("Masking: applying mask to fish/host stacks (current + future)\n")
  ...
  # cell area for km² metrics downstream
  cell_area_km2 <- terra::cellSize(template_r, unit = "km")
  cat("Masking: cell_area_km2 computed.\n")
}

cat("--- Masking (MEOW + depth cutoff): finished ---\n")
...
#| message: false
#| warning: false
#| fig-cap: "Final Conservation Plots (masked to MEOW + -750m depth)"
...
required_objs <- c(
  ...
  "conservation_display_bbox",
  "cell_area_km2"
)
missing <- required_objs[!vapply(required_objs, exists, logical(1))]
if (length(missing) > 0) {
  stop("Missing required objects: ", paste(missing, collapse = ", "),
       "\nMake sure you've run: build-fish-occ-all -> masking-plots -> this chunk.")
}

scenario_hi <- "ssp585_2100"
if (!(scenario_hi %in% names(fish_pred_stacks_future_individual))) stop("scenario_hi not found in fish future stacks: ", scenario_hi)
if (!(scenario_hi %in% names(host_pred_stacks_future_individual))) stop("scenario_hi not found in host future stacks: ", scenario_hi)

# Output dirs
conservation_dir <- file.path(figure_output_dir, "conservation")
species_delta_dir <- file.path(conservation_dir, "species_delta_only")
dir.create(conservation_dir, recursive = TRUE, showWarnings = FALSE)
dir.create(species_delta_dir, recursive = TRUE, showWarnings = FALSE)
...
crop_for_display <- function(r, bbox) {
  terra::crop(r, terra::ext(bbox))
}
...
stretch01_quantile <- function(r, q = 0.99, power = 0.5) {
  qv <- as.numeric(terra::global(r, fun = function(x) stats::quantile(x, probs = q, na.rm = TRUE))[1,1])
  if (is.na(qv) || qv <= 0) return(r * 0)
  out <- r / qv
  out <- terra::clamp(out, lower = 0, upper = 1, values = TRUE)
  if (!is.null(power) && is.finite(power) && power != 1) out <- out ^ power
  out
}

q_global <- function(r, p = 0.8) {
  as.numeric(terra::global(r, fun = function(x) stats::quantile(x, probs = p, na.rm = TRUE))[1,1])
}

plot_index_map <- function(index_raster_01, title, world_basemap, legend_title = "Index (0–1)") {
  r <- crop_for_display(index_raster_01, conservation_display_bbox)
  names(r) <- "Index"
  ggplot() +
    geom_sf(data = world_basemap, fill = "grey80", color = "white", linewidth = 0.2) +
    geom_spatraster(data = r, aes(fill = Index)) +
    ...
    coord_sf(
      ...
      xlim = terra::ext(r)[c(1,2)],
      ylim = terra::ext(r)[c(3,4)],
      expand = FALSE
    ) +
    labs(title = title) +
    theme_minimal() +
...
plot_delta_map_threshold <- function(delta_raster, title, world_basemap,
                                    thr_white = 0.25, lim_quant = 0.99,
                                    legend_title = "Δ suitability") {
  r <- crop_for_display(delta_raster, conservation_display_bbox)
  r_thr <- terra::ifel(abs(r) < thr_white, 0, r)
  names(r_thr) <- "Delta"

  abs_lim <- as.numeric(terra::global(abs(r_thr),
                                     fun = function(x) stats::quantile(x, probs = lim_quant, na.rm = TRUE))[1,1])
  abs_lim <- max(thr_white, abs_lim, na.rm = TRUE)
  if (!is.finite(abs_lim) || abs_lim <= 0) abs_lim <- thr_white

  ggplot() +
    geom_sf(data = world_basemap, fill = "grey80", color = "white", linewidth = 0.2) +
    geom_spatraster(data = r_thr, aes(fill = Delta)) +
    ...
    coord_sf(
      crs = sf::st_crs(terra::crs(r_thr)),
      xlim = terra::ext(r_thr)[c(1,2)],
      ylim = terra::ext(r_thr)[c(3,4)],
      expand = FALSE
    ) +
    labs(title = title) +
    theme_minimal() +
...
# -----------------------------
# 2) Policy-facing conservation surfaces (masked already)
# -----------------------------
fish_rich_current <- if (exists("fish_richness_sum_cropped")) fish_richness_sum_cropped else terra::app(fish_pred_stack_cropped, sum, na.rm = TRUE)
host_rich_current <- if (exists("host_richness_sum_cropped")) host_richness_sum_cropped else terra::app(host_pred_stack_cropped, sum, na.rm = TRUE)

fish_rich_future_hi <- terra::app(fish_pred_stacks_future_individual[[scenario_hi]], sum, na.rm = TRUE)
host_rich_future_hi <- terra::app(host_pred_stacks_future_individual[[scenario_hi]], sum, na.rm = TRUE)

fish_rich_future_hi <- safe_align_to(fish_rich_future_hi, fish_rich_current)
host_rich_future_hi <- safe_align_to(host_rich_future_hi, host_rich_current)

fish_loss_hi <- terra::ifel(fish_rich_current > fish_rich_future_hi, fish_rich_current - fish_rich_future_hi, 0)
host_loss_hi <- terra::ifel(host_rich_current > host_rich_future_hi, host_rich_current - host_rich_future_hi, 0)

fish_rich_cur_n <- normalize01(fish_rich_current)
fish_loss_n     <- normalize01(fish_loss_hi)
host_loss_n     <- normalize01(host_loss_hi)
host_rich_fut_n <- normalize01(host_rich_future_hi)
fish_rich_fut_n <- normalize01(fish_rich_future_hi)

cpi_hi_raw <- normalize01(fish_rich_cur_n * fish_loss_n * (0.5 + 0.5 * host_loss_n))
refugia_hi <- normalize01(fish_rich_fut_n * host_rich_fut_n)

cpi_hi_disp <- stretch01_quantile(cpi_hi_raw, q = 0.99, power = 0.5)

cpi_title <- paste0(
  "Conservation Priority Index (Hotspots of Projected Loss)\nScenario: ",
  scenario_label_converter[[scenario_hi]] %||% scenario_hi
)
refugia_title <- paste0(
  "Mutualism Refugia (Future Co-persistence of Fish + Hosts)\nScenario: ",
  scenario_label_converter[[scenario_hi]] %||% scenario_hi
)

p_cpi <- plot_index_map(cpi_hi_disp, cpi_title, world_map_sf, legend_title = "Priority (stretched 0–1)")
p_ref <- plot_index_map(refugia_hi, refugia_title, world_map_sf, legend_title = "Refugia (0–1)")

ggsave(file.path(conservation_dir, paste0("CPI_", scenario_hi, ".png")), p_cpi, width = 11, height = 7, dpi = 300, bg = "white")
ggsave(file.path(conservation_dir, paste0("Refugia_", scenario_hi, ".png")), p_ref, width = 11, height = 7, dpi = 300, bg = "white")

print(p_cpi)
print(p_ref)

# -----------------------------
# 3) Country-level summary (still fine; uses masked rasters)
# -----------------------------
cpi_coarse <- terra::aggregate(cpi_hi_raw, fact = 5, fun = mean, na.rm = TRUE)
ref_coarse <- terra::aggregate(refugia_hi, fact = 5, fun = mean, na.rm = TRUE)
...
country_summary <- countries_sf %>%
  dplyr::mutate(.id = dplyr::row_number()) %>%
  sf::st_drop_geometry() %>%
  dplyr::select(.id, name_long = name_long, name = name) %>%
  dplyr::left_join(dplyr::tibble(.id = cpi_by_country$ID, mean_CPI = cpi_by_country[,2]), by = ".id") %>%
  dplyr::left_join(dplyr::tibble(.id = ref_by_country$ID, mean_Refugia = ref_by_country[,2]), by = ".id") %>%
  dplyr::mutate(mean_CPI = as.numeric(mean_CPI), mean_Refugia = as.numeric(mean_Refugia)) %>%
  dplyr::filter(!is.na(mean_CPI) | !is.na(mean_Refugia)) %>%
  dplyr::arrange(dplyr::desc(mean_CPI))

write.csv(country_summary, file.path(conservation_out_csv, paste0("country_summary_CPI_Refugia_", scenario_hi, ".csv")), row.names = FALSE)

cat("\nTop countries by mean CPI (coarse):\n")
print(knitr::kable(head(country_summary, 15), digits = 3))

# -----------------------------
# 4) Species-level habitat loss + DELTA-ONLY maps (masked already)
# -----------------------------
fish_curr_stack <- fish_pred_stack_cropped
fish_fut_stack  <- fish_pred_stacks_future_individual[[scenario_hi]]

common_species <- intersect(names(fish_curr_stack), names(fish_fut_stack))
if (length(common_species) == 0) stop("No overlapping species between current and future fish stacks for ", scenario_hi)

fish_curr_stack <- fish_curr_stack[[common_species]]
fish_fut_stack  <- fish_fut_stack[[common_species]]

aligned_future_layers <- list()
for (sp in common_species) aligned_future_layers[[sp]] <- safe_align_to(fish_fut_stack[[sp]], fish_curr_stack[[sp]])
...
species_metrics <- lapply(common_species, function(sp) {
  ...
  if (is.na(thr)) thr <- 0.8

  mask_cur <- terra::ifel(cur >= thr, 1, 0)
  mask_fut <- terra::ifel(fut >= thr, 1, 0)

  area_cur <- as.numeric(terra::global(mask_cur * cell_area_km2, "sum", na.rm = TRUE)[1,1])
  area_fut <- as.numeric(terra::global(mask_fut * cell_area_km2, "sum", na.rm = TRUE)[1,1])

  refugia_mask <- terra::ifel((cur >= thr) & (fut >= thr), 1, 0)
  loss_mask    <- terra::ifel((cur >= thr) & (fut <  thr), 1, 0)

  area_refugia <- as.numeric(terra::global(refugia_mask * cell_area_km2, "sum", na.rm = TRUE)[1,1])
  area_loss    <- as.numeric(terra::global(loss_mask    * cell_area_km2, "sum", na.rm = TRUE)[1,1])
...
write.csv(
  species_metrics_df,
  file.path(conservation_out_csv, paste0("species_core_habitat_metrics_", scenario_hi, ".csv")),
  row.names = FALSE
)

cat("\nWorst-hit species by % core habitat lost (", scenario_hi, "):\n", sep = "")
print(knitr::kable(head(species_metrics_df, 12), digits = 2))

p_loss_bar <- ggplot(species_metrics_df, aes(x = reorder(species_display, pct_core_lost), y = pct_core_lost)) +
  geom_col() +
  coord_flip() +
  labs(
...
ggsave(file.path(conservation_dir, paste0("species_core_habitat_loss_bar_", scenario_hi, ".png")),
       p_loss_bar, width = 10, height = 8, dpi = 300, bg = "white")
print(p_loss_bar)

n_print <- min(6, nrow(species_metrics_df))
worst_species <- species_metrics_df$species[1:n_print]

for (sp in worst_species) {
  cur <- fish_curr_stack[[sp]]
  fut <- fish_fut_stack_aligned[[sp]]
  delta <- fut - cur

  p_del <- plot_delta_map_threshold(
    delta,
    paste0(gsub("_", " ", sp), " — Δ (Future - Current)\n", scenario_label_converter[[scenario_hi]] %||% scenario_hi),
    world_map_sf,
    thr_white = 0.25,
    lim_quant = 0.99,
    legend_title = "Δ suitability"
  ...
  out_file <- file.path(species_delta_dir, paste0(sp, "_DELTA_ONLY_", scenario_hi, ".png"))
  ggsave(out_file, p_del, width = 7.5, height = 6, dpi = 300, bg = "white")
  print(p_del)
}

cat("\nSaved DELTA-only maps to: ", species_delta_dir, "\n", sep = "")
cat("Saved CPI/Refugia + summaries to: ", conservation_dir, "\n", sep = "")
cat("--- Final Conservation Plots: finished ---\n")
```
