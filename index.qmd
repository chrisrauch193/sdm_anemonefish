...
# Reusable function to create publication-quality DIFFERENCE maps
# This uses a diverging color palette to show gains and losses.
plot_difference_map <- function(difference_raster, map_title, world_basemap) {
  
  # Ensure the raster has a name for the legend
  names(difference_raster) <- "SuitabilityChange"
  
  # Find the maximum absolute change to center the color scale around zero
  max_abs_change <- max(abs(minmax(difference_raster)), na.rm = TRUE)
  ...
  p <- ggplot() +
    ...
    # 2. Add the difference raster on top
    geom_spatraster(data = difference_raster, aes(fill = SuitabilityChange)) +
    
    # 3. Apply a DIVERGING color scale
    # This scale is centered on zero. Blue = decrease, Grey = no change, Red = increase.
    scale_fill_gradient2(
      low = "blue", 
      mid = "white", 
      high = "red",
      midpoint = 0,
      limits = c(-max_abs_change, max_abs_change), # Symmetrical limits
      na.value = "transparent",
      name = "Change in\nSuitability"
    ) +
    
    # 4. Set map projection and limits
    coord_sf(crs = sf::st_crs(difference_raster),
             xlim = ext(difference_raster)[c(1,2)],
             ylim = ext(difference_raster)[c(3,4)],
             expand = FALSE) +
...
if (terra::nlyr(host_pred_stack_cropped) > 0) {
  for (i in 1:terra::nlyr(host_pred_stack_cropped)) {
    ...
    if (exists("plot_prediction_map") && exists("world_map_sf")) {
      ...
      print(host_plot) # Display the plot
    } else {
      # Fallback to original plot if helper function/world map isn't available
