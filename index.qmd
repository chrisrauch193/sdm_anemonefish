...
This section statistically compares the performance (test AUC) of the different anemonefish model types (environmental-only, biotic-only, combined host+environment) using a linear mixed model. It also visualizes these comparisons and performs pairwise comparisons at the species level.
...
if (!is.null(df_fish_comparison) && nrow(df_fish_comparison) > 0 && "test_AUC" %in% names(df_fish_comparison)) {
  ...
  # --- Statistical Comparison using GLMM (Overall Effect) ---
  cat("\n--- Fitting GLMM to compare test_AUC across model types (Overall Effect) ---\n")
  m1_fish_auc <- tryCatch({
    glmmTMB(test_AUC ~ model_type + (1|species), data = df_fish_comparison)
  }, error = function(e) {
    cat("Error fitting GLMM for AUC:", e$message, "\n")
  ...
  if (!is.null(m1_fish_auc)) {
    cat("\nSummary of GLMM for test_AUC:\n")
    print(summary(m1_fish_auc))
    # print(performance::model_performance(m1_fish_auc)) # Optional
  }

  # --- Visualization (Overall and Per Species) ---
  # (Your existing ggplot code for plot_auc_comparison and plot_auc_per_species remains here)
  cat("\n--- Generating boxplot of test_AUC by model type ---\n")
  # ... (plot_auc_comparison code from your previous QMD) ...
  plot_auc_comparison <- ggplot(df_fish_comparison, aes(x = model_type, y = test_AUC, fill = model_type)) +
    geom_boxplot(alpha = 0.7) +
    stat_summary(fun = mean, geom = "point", shape = 18, size = 3, color = "black",
  ...
  # --- Per-Species Pairwise Comparison of Model Performance (AUC) ---
  cat("\n\n--- Per-Species Pairwise Comparison of Model Performance (AUC) ---\n")
  
  # Get unique species
  unique_species_fish <- unique(df_fish_comparison$species)
  
  all_species_test_results <- list() # To store results of tests
  
  for (sp in unique_species_fish) {
    cat("\n  -- Species:", sp, "--\n")
    species_data <- df_fish_comparison %>% filter(species == sp)
    
    # Ensure there's data for at least two model types for this species
    if (length(unique(species_data$model_type)) < 2) {
      cat("    Skipping: Not enough model types with data for this species.\n")
      next
    }
    
    # Define the comparisons you want to make
    # For example: env_only vs. combined_host_env, env_only vs. biotic_only
    comparisons_to_make <- list(
      c("env_only", "combined_host_env"),
      c("env_only", "biotic_only"),
      c("biotic_only", "combined_host_env")
    )
    
    species_results_current <- list()
    
    for (comp_pair in comparisons_to_make) {
      type1 <- comp_pair[1]
      type2 <- comp_pair[2]
      
      data_type1 <- species_data %>% filter(model_type == type1) %>% pull(test_AUC)
      data_type2 <- species_data %>% filter(model_type == type2) %>% pull(test_AUC)
      
      # Ensure we have enough data points (e.g., from multiple CV folds) for each type
      # And that the number of data points are equal for a paired test.
      # If your SDMtune runs generate a fixed number of CV results (e.g., 40 like the ant paper),
      # these lengths should typically be equal.
      if (length(data_type1) > 1 && length(data_type2) > 1 && length(data_type1) == length(data_type2)) {
        
        # Paired t-test (assumes differences are normally distributed)
        # Alternatively, Wilcoxon signed-rank test (non-parametric)
        # test_result <- t.test(data_type1, data_type2, paired = TRUE, alternative = "two.sided")
        test_result <- tryCatch({
            wilcox.test(data_type1, data_type2, paired = TRUE, alternative = "two.sided", exact = FALSE, correct = TRUE)
        }, warning = function(w){ # Handle warnings, e.g., about ties for Wilcoxon
            cat("      Warning during Wilcoxon test for", sp, type1, "vs", type2, ":", w$message, "\n")
            # Attempt to run again without continuity correction if it's a common warning source with ties
            tryCatch({
                wilcox.test(data_type1, data_type2, paired = TRUE, alternative = "two.sided", exact = FALSE, correct = FALSE)
            }, error = function(e2) { cat("      Wilcoxon test (no correction) also failed:", e2$message, "\n"); NULL})
        }, error = function(e) {
            cat("      Error in Wilcoxon test for", sp, type1, "vs", type2, ":", e$message, "\n")
            NULL
        })

        if (!is.null(test_result)) {
          cat("    Comparison:", type1, "vs.", type2, "\n")
          # print(broom::tidy(test_result)) # broom makes output tidy
          cat(sprintf("      Method: %s, W = %.2f, p-value = %.4f\n", test_result$method, test_result$statistic, test_result$p.value))
          cat(sprintf("      Mean AUC %s: %.3f, Mean AUC %s: %.3f\n", type1, mean(data_type1, na.rm=T), type2, mean(data_type2, na.rm=T)))
          species_results_current[[paste0(type1, "_vs_", type2)]] <- list(
            method = test_result$method,
            statistic = test_result$statistic,
            p.value = test_result$p.value,
            mean_auc_type1 = mean(data_type1, na.rm=T),
            mean_auc_type2 = mean(data_type2, na.rm=T)
          )
        }
      } else {
        cat("    Skipping comparison", type1, "vs.", type2, "- insufficient or unequal data points.\n")
        cat("      Data points for", type1, ":", length(data_type1), "; for", type2, ":", length(data_type2), "\n")
      }
    } # End loop through comparison pairs
    all_species_test_results[[sp]] <- species_results_current
  } # End loop through species
  
  # You can further process `all_species_test_results` to create a summary table of p-values if desired.
  # For example, to see how many species showed significant improvement with combined models:
  if(length(all_species_test_results) > 0) {
      cat("\n\n--- Summary of Per-Species Test Significances (p < 0.05) ---\n")
      for(comp_name in names(all_species_test_results[[1]])){ # Assuming all species have same comparisons
          significant_count <- 0
          better_type2_count <- 0 # Count where type2 was significantly better
          cat("  Comparison:", comp_name, "\n")
          for(sp_name_res in names(all_species_test_results)){
              if(!is.null(all_species_test_results[[sp_name_res]][[comp_name]]$p.value) &&
                 all_species_test_results[[sp_name_res]][[comp_name]]$p.value < 0.05){
                  significant_count <- significant_count + 1
                  # Check which mean AUC was higher
                  mean1 <- all_species_test_results[[sp_name_res]][[comp_name]]$mean_auc_type1
                  mean2 <- all_species_test_results[[sp_name_res]][[comp_name]]$mean_auc_type2
                  comp_pair_names <- strsplit(comp_name, "_vs_")[[1]]
                  if (mean2 > mean1) {
                      cat("    ", sp_name_res, ":", comp_pair_names[2], "significantly better than", comp_pair_names[1], "(p =", round(all_species_test_results[[sp_name_res]][[comp_name]]$p.value, 4), ")\n")
                      better_type2_count <- better_type2_count + 1
                  } else if (mean1 > mean2) {
                      cat("    ", sp_name_res, ":", comp_pair_names[1], "significantly better than", comp_pair_names[2], "(p =", round(all_species_test_results[[sp_name_res]][[comp_name]]$p.value, 4), ")\n")
                  } else {
                      cat("    ", sp_name_res, ": Significant difference, but means are equal (p =", round(all_species_test_results[[sp_name_res]][[comp_name]]$p.value, 4), ") - check data.\n")
                  }
              }
          }
          cat("    Total species with significant difference for", comp_name, ":", significant_count, "\n")
          if (grepl("combined_host_env", comp_name)) { # If this comparison involves combined
             cat("    Total species where combined model was significantly better:", better_type2_count, "\n")
          }
      }
  }
...
# (TSS analysis section would follow similarly if added later)
```
...
if (nrow(host_niche_breadth_df) > 0) {
  ...
  for (scenario_name in future_scenarios_to_load) {
    ...
    if (shift_col_name %in% names(host_breadth_shifts_df)) {
      ...
      temp_df_cor_host <- host_breadth_shifts_df %>% dplyr::select(breadth_B2, !!sym(shift_col_name)) %>% na.omit()
...
if (nrow(fish_niche_breadth_df) > 0) {
  ...
  for (scenario_name in future_scenarios_to_load) {
    ...
    if (shift_col_name %in% names(fish_breadth_shifts_df)) {
      temp_df_cor_fish <- fish_breadth_shifts_df %>% dplyr::select(breadth_B2, !!sym(shift_col_name)) %>% na.omit()
...
#| fig-width: 10
#| fig-height: 5 # Adjust for two plots side-by-side
#| message: false
#| warning: false
...
# This should match one of the columns created in host_breadth_shifts_df and fish_breadth_shifts_df
# e.g., "shift_585_2100" (derived from "ssp585_2100")
chosen_scenario_for_figure <- "shift_585_2100" # CHANGE AS NEEDED

plot_a_host <- NULL
plot_b_fish <- NULL

# --- Host Anemones Plot ---
if (!is.null(host_breadth_shifts_df) && chosen_scenario_for_figure %in% names(host_breadth_shifts_df) && "breadth_B2" %in% names(host_breadth_shifts_df)) {
  
  # Check correlation for the dashed line logic (significant vs. non-significant)
  cor_data_host <- host_breadth_shifts_df %>% dplyr::select(breadth_B2, !!sym(chosen_scenario_for_figure)) %>% na.omit()
  host_cor_test <- NULL
  if(nrow(cor_data_host) > 2) {
    host_cor_test <- cor.test(cor_data_host$breadth_B2, cor_data_host[[chosen_scenario_for_figure]])
  }
  
  plot_a_host <- ggplot(host_breadth_shifts_df, aes(x = .data[[chosen_scenario_for_figure]], y = breadth_B2)) +
    geom_point(alpha = 0.8, color = "dodgerblue3") +
    geom_smooth(method = "lm", 
                linetype = if (!is.null(host_cor_test) && host_cor_test$p.value < 0.05) "solid" else "dashed", 
                se = TRUE, color = "black") + # Show SE, solid if p < 0.05
    labs(
      # title = "Host Anemones",
  ...
  cat("Skipping host breadth vs. shift plot: data for", chosen_scenario_for_figure, "or breadth_B2 missing.\n")
}

# --- Anemonefish (Env-Only) Plot ---
if (!is.null(fish_breadth_shifts_df) && chosen_scenario_for_figure %in% names(fish_breadth_shifts_df) && "breadth_B2" %in% names(fish_breadth_shifts_df)) {
  
  cor_data_fish <- fish_breadth_shifts_df %>% dplyr::select(breadth_B2, !!sym(chosen_scenario_for_figure)) %>% na.omit()
  fish_cor_test <- NULL
  if(nrow(cor_data_fish) > 2) {
    fish_cor_test <- cor.test(cor_data_fish$breadth_B2, cor_data_fish[[chosen_scenario_for_figure]])
  }

  plot_b_fish <- ggplot(fish_breadth_shifts_df, aes(x = .data[[chosen_scenario_for_figure]], y = breadth_B2)) +
    geom_point(alpha = 0.8, color = "firebrick3") +
    geom_smooth(method = "lm", 
                linetype = if (!is.null(fish_cor_test) && fish_cor_test$p.value < 0.05) "solid" else "dashed", 
                se = TRUE, color = "black") +
    labs(
      # title = "Anemonefish (Env-Only)",
      x = paste("Mean Suitability Shift (", gsub("shift_", "SSP", chosen_scenario_for_figure), ")"),
      y = "Environmental Breadth (Levin's B2)"
    ) +
    theme_bw(base_size = 11) +
  ...
  cat("Anemonefish plot generated for", chosen_scenario_for_figure, "\n")
  if(!is.null(fish_cor_test)) print(fish_cor_test)
  
} else {
  cat("Skipping anemonefish breadth vs. shift plot: data for", chosen_scenario_for_figure, "or breadth_B2 missing.\n")
}

# --- Combine Plots ---
if (!is.null(plot_a_host) && !is.null(plot_b_fish)) {
  combined_breadth_plot <- cowplot::plot_grid(plot_a_host, plot_b_fish, ncol = 2, labels = "AUTO")
  print(combined_breadth_plot)
...
host_vi_subdir_name <- config$model_output_subdir_map[[env_model_predictor_suffix]] %||% ifelse(config$use_pca_predictors, "anemone_pca", "anemone_vif") # Fallback
host_vi_dir <- file.path(config$target_vi_base, host_vi_subdir_name) # target_vi_base is data/output

host_vi_files <- list.files(path = host_vi_dir, pattern = "^VI_.*\\.csv$", full.names = TRUE)
if (length(host_vi_files) == 0 && env_model_predictor_suffix == "_pca") { # Specific check for typical pca subfolder in target_vi_base
    host_vi_dir_alt <- file.path(config$target_vi_base, "anemone_pca")
    host_vi_files <- list.files(path = host_vi_dir_alt, pattern = "^VI_.*\\.csv$", full.names = TRUE)
    if(length(host_vi_files)>0) host_vi_dir <- host_vi_dir_alt # Update if found in alt
}
...
if (length(host_vi_files) > 0) {
  host_vi_data <- purrr::map_df(host_vi_files, 
    ...
    rename(Permutation_Importance = Importance) # Match ant paper column if different
    
  cat("Loaded VI data for", length(unique(host_vi_data$species_sanitized)), "host species.\n")
} else {
  cat("Warning: No Variable Importance CSV files found for host anemones in:", host_vi_dir, "\n")
  host_vi_data <- NULL
}

# --- Load Anemonefish (Environmental-Only Models) Variable Importance ---
cat("\n--- Loading Anemonefish (Env-Only) Variable Importance ---\n")
fish_vi_subdir_name <- config$model_output_subdir_map[[env_model_predictor_suffix]] %||% ifelse(config$use_pca_predictors, "anemonefish_pca", "anemonefish_vif")
fish_vi_dir <- file.path(config$target_vi_base, fish_vi_subdir_name)

fish_vi_files <- list.files(path = fish_vi_dir, pattern = "^VI_.*\\.csv$", full.names = TRUE)
if (length(fish_vi_files) == 0 && env_model_predictor_suffix == "_pca") { # Specific check
    fish_vi_dir_alt <- file.path(config$target_vi_base, "anemonefish_pca")
    fish_vi_files <- list.files(path = fish_vi_dir_alt, pattern = "^VI_.*\\.csv$", full.names = TRUE)
    if(length(fish_vi_files)>0) fish_vi_dir <- fish_vi_dir_alt
}
...
if (length(fish_vi_files) > 0) {
  fish_vi_data <- purrr::map_df(fish_vi_files, 
    ...
    mutate(
      species_sanitized = str_remove(str_remove(filename, "^VI_"), "\\.csv$"),
      guild = "Anemonefish_EnvOnly"
    ) %>%
    rename(Permutation_Importance = Importance) # Match ant paper column if different
  cat("Loaded VI data for", length(unique(fish_vi_data$species_sanitized)), "anemonefish (env-only) species.\n")
} else {
  cat("Warning: No Variable Importance CSV files found for anemonefish (env-only) in:", fish_vi_dir, "\n")
  fish_vi_data <- NULL
}
...
if (!is.null(host_vi_data) && !is.null(fish_vi_data) && nrow(host_vi_data) > 0 && nrow(fish_vi_data) > 0) {
  combined_vi_data <- bind_rows(host_vi_data, fish_vi_data) %>%
    filter(!is.na(Permutation_Importance)) # Ensure importance values are valid
  
  # Ensure 'Variable' column exists (it should from SDMtune::varImp output)
  if (!"Variable" %in% names(combined_vi_data)) {
    stop("The 'Variable' column is missing from the loaded VI data. Check the output of calculate_and_save_vi.")
  }
  
  cat("\nCombined VI data for comparison:\n")
  print(head(combined_vi_data))

  # Boxplot of VI by guild
  plot_vi_comparison <- ggplot(combined_vi_data, aes(x = Variable, y = Permutation_Importance, fill = guild)) +
    ...
    scale_fill_manual(values = c("Host_Anemones" = "coral", "Anemonefish_EnvOnly" = "steelblue"), name = "Guild") +
    theme_bw(base_size = 12) +
    theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position = "top")
  
  print(plot_vi_comparison)
  
  # Perform t-tests for each predictor variable (e.g., each PC component if PCA was used)
  ...
  for (var_name in predictor_vars_to_test) {
    ...
    if (nrow(data_for_ttest %>% distinct(guild)) == 2 && 
        nrow(data_for_ttest %>% filter(guild == "Host_Anemones") %>% na.omit()) > 1 &&
        nrow(data_for_ttest %>% filter(guild == "Anemonefish_EnvOnly") %>% na.omit()) > 1) {
      ...
      cat("    Skipping t-test for", var_name, "- insufficient data or only one guild present.\n")
    }
  }
