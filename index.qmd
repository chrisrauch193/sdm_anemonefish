...
plot_prediction_map <- function(prediction_raster, species_name, world_basemap) {
  ...
  # It goes from a light yellow-ish color to orange to deep purple
  custom_color_scale <- scale_fill_gradientn(
    colors = c("#FFFFFF", "#FEF9C3", "#FDBA74", "#D946EF", "#4F46E5"), # Light Yellow, Orange, Pink/Purple, Deep Purple
    na.value = "transparent", # IMPORTANT: Makes areas with no prediction transparent
    limits = c(0, 1), # Assuming suitability is 0-1
    name = "Occurrence\nprobability"
  )
  ...
  p <- ggplot() +
    ...
    coord_sf(crs = sf::st_crs(prediction_raster),
...
cat("\n--- Plotting Cropped Richness Maps with ggplot2 ---\n")

# Plot Host Anemone Richness
if (!is.null(host_richness_sum_cropped)) {
  # The richness values might be > 1, so we need to normalize them to 0-1 for the color scale
  host_richness_normalized <- host_richness_sum_cropped / global(host_richness_sum_cropped, "max", na.rm = TRUE)$max

  host_richness_plot <- plot_prediction_map(
    prediction_raster = host_richness_normalized,
    species_name = "Summed Host Anemone Richness",
  ...
  # Save the plot
  host_plot_filename <- file.path(figure_output_dir, "host_richness_current_ggplot.png")
  ggsave(host_plot_filename, plot = host_richness_plot, width = 10, height = 7, dpi = 300)
  cat("Saved host richness ggplot to:", host_plot_filename, "\n")
}


# Plot Anemonefish (Env-Only) Richness
if (!is.null(fish_richness_sum_cropped)) {
  fish_richness_normalized <- fish_richness_sum_cropped / global(fish_richness_sum_cropped, "max", na.rm = TRUE)$max

  fish_richness_plot <- plot_prediction_map(
    prediction_raster = fish_richness_normalized,
    species_name = "Summed Anemonefish Richness (Env-Only)",
    world_basemap = world_map_sf
  )
  ...
  # Save the plot
  fish_plot_filename <- file.path(figure_output_dir, "fish_richness_env_only_ggplot.png")
  ggsave(fish_plot_filename, plot = fish_richness_plot, width = 10, height = 7, dpi = 300)
  cat("Saved fish richness ggplot to:", fish_plot_filename, "\n")
}

# 
# # --- PLOT INDIVIDUAL SPECIES TO REPLICATE THE EXAMPLE ---
# # Let's plot the first anemone to demonstrate
# if (!is.null(host_pred_stack_cropped)) {
#   
#   first_anemone_raster <- host_pred_stack_cropped[[1]] # Get the first layer
#   first_anemone_name <- names(first_anemone_raster)   # Get its name
#   
#   # The species name might be like "Radianthus_aurora", so we make it pretty for the title
#   pretty_species_name <- gsub("_", " ", first_anemone_name) 
#   
#   # Create the plot
#   individual_species_plot <- plot_prediction_map(
#     prediction_raster = first_anemone_raster,
#     species_name = pretty_species_name,
#     world_basemap = world_map_sf
#   )
#   
#   print(individual_species_plot)
#   
#   # Save it
#   individual_plot_filename <- file.path(figure_output_dir, paste0(first_anemone_name, "_prediction_map.png"))
#   ggsave(individual_plot_filename, plot = individual_species_plot, width = 10, height = 7, dpi = 300)
#   cat("Saved individual species map to:", individual_plot_filename, "\n")
# }
# 
# 
# cat("\n--- First section of results processing finished. ---\n")
...
host_cv_files <- list.files(host_cv_results_dir,
                            pattern = "^CV_Results_.*_depth_-500\\.csv$", # Files start with CV_Results_
                            full.names = TRUE)
...
fish_env_cv_files <- list.files(fish_env_cv_results_dir,
                                pattern = "^CV_Results_.*_depth_-500\\.csv$",
                                full.names = TRUE)
...
if (!is.null(df_fish_comparison) && nrow(df_fish_comparison) > 0 && "AUC_test_CV" %in% names(df_fish_comparison)) {
  ...
  for (current_species_name_sanitized in all_anemonefish_species_for_loop) {
    ...
    species_specific_figure_output_dir <- paste0(figure_output_dir, "/AUC_boxplots")
    
    
    if (!dir.exists(species_specific_figure_output_dir)) {
      dir.create(species_specific_figure_output_dir, recursive = TRUE)
      cat("Created directory for figures at:", species_specific_figure_output_dir, "\n")
    }
    
    # Define filename for host richness plot
    host_plot_filename <- file.path(species_specific_figure_output_dir, paste0("AUC_boxplot_", current_species_name_sanitized, ".png"))
    ...
    # Close PNG device
    dev.off()
    cat("Saved species specific plot to:", host_plot_filename, "\n")
...
# current_host_fish_overlap_raw <- overlap_current_matrix_d[valid_fish_cols, valid_host_rows, drop = FALSE]
# 
...
#   }
# 
#   future_host_fish_overlap_raw_chosen <- overlap_future_chosen_matrix[valid_fish_cols, valid_host_rows, drop = FALSE]
# 
...
#       dplyr::select(host_species, fish_species, proportional_change_capped)
# 
